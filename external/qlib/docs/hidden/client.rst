...
qlib ``init`` commonly used parameters; ``nfs-common`` must be installed on the server where the client is located, execute: ``sudo apt install nfs-common``:
    - ``provider_uri``: nfs-server path; the format is ``host: data_dir``, for example: ``192.0.2.1:/data2/gaochao/sync_qlib/qlib``. If using offline, it can be a local data directory
    ...
    - ``flask_server``: data service host; if you are on the intranet, you can use the default host: 192.0.2.1
    - ``flask_port``: data service port


If running on 192.0.2.1 or 192.0.2.1 server, it's recommended to use the following code to ``init`` qlib:
...
.. code-block:: python
   ...
   >>> D.features(['SH600000'], ['$close'], start_time='20080101', end_time='20090101').head()
    ...
    [39336:MainThread](2019-05-28 21:35:42,801) INFO - Initialization - [__init__.py:56] - provider_uri=192.0.2.1:/data2/gaochao/sync_qlib/qlib
    [39336:Thread-68](2019-05-28 21:35:42,809) INFO - Client - [client.py:28] - Connect to server ws://192.0.2.1:9710
...
.. code-block:: python
   ...
   >>> import qlib
   >>> qlib.init(auto_mount=True, mount_path='/data/csdesign/qlib', provider_uri='192.0.2.1:/data2/gaochao/sync_qlib/qlib')
   >>> from qlib.data import D
   >>> D.features(['SH600000'], ['$close'], start_time='20080101', end_time='20090101').head()
    [39336:MainThread](2019-05-28 21:35:42,800) INFO - Initialization - [__init__.py:16] - default_conf: client.
    [39336:MainThread](2019-05-28 21:35:42,801) INFO - Initialization - [__init__.py:54] - qlib successfully initialized based on client settings.
...
.. code-block:: python
   ...
   >>> import qlib
   >>> qlib.init(auto_mount=True, mount_path='H', provider_uri='192.0.2.1:/data2/gaochao/sync_qlib/qlib')
   >>> from qlib.data import D
   >>> D.features(['SH600000'], ['$close'], start_time='20080101', end_time='20090101').head()
    [39336:MainThread](2019-05-28 21:35:42,800) INFO - Initialization - [__init__.py:16] - default_conf: client.
    [39336:MainThread](2019-05-28 21:35:42,801) INFO - Initialization - [__init__.py:54] - qlib successfully initialized based on client settings.
