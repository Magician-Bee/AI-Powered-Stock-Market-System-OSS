...
> 首次公開原始碼版本，由 Magician-Bee 持續維護。原創程式碼採 [MIT 授權](LICENSE)；第三方元件保留各自授權。請先閱讀 [公開版本說明](PUBLIC_RELEASE.md) 與 [第三方聲明](THIRD_PARTY_NOTICES.md)。目前尚無外部使用者或下載統計。

> 公開版未包含 AI-Trader 的第三方原始碼及私人開發驗收截圖；需要 AI-Trader 的選用流程會提示缺少前置條件。其他功能與尚待驗收項目以下方說明為準。

以台股為核心的本機市場研究、可替換模型 Agent 分析與紙上交易工作站。

本專案整合官方市場資料、即時行情、新聞事件、基本面、籌碼、策略研究、中央風控、模擬委託與持倉帳本。系統目前定位為研究與模擬交易平台，不保證獲利，也不會向真實券商送單。
...
完整 P0–P105 release checklist：**83/124**；P1/P2 未驗收項目同樣會阻擋 release。
...
- 台股即時行情、可重建 1／5／15／30／60 分 K、完整日期範圍歷史日 K（原始／前復權／後復權、OHLCV／成交金額）、新聞與事件
...
- FinRL 已完成真實 policy 訓練／保存／重新載入／推論，Qlib 已完成 Dataset／Model／Recorder／Signal／Portfolio Analysis workflow
