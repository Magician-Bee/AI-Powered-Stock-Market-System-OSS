...
The public distribution starts a new Git history and has an empty
`.gitleaksignore` baseline. Imported example credentials are replaced with
placeholders, test fixtures use low-entropy identifiers, and AI-Trader source
is not bundled. Any new finding fails the scan. The historical audit records
below describe the earlier private development repository, not this public
distribution or a credential-rotation receipt.
