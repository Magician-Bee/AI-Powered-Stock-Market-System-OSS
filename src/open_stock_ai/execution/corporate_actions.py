...
class CorporateActionLedger:
    ...
    def __post_init__(self) -> None:
        ...
        if retention_path is None or Path(retention_path).resolve() != Path(self.store.path).resolve():
            raise ValueError("corporate action retention must share its durable database")
    ...
    def apply_to_account(self, *, item: dict[str, Any], account_id: str) -> dict[str, Any]:
        ...
        with self.store._connect() as conn:
            ...
            if existing is not None:
                conn.commit()
                result = self._serialize_application(existing, idempotent=True)
                self._retain_application(result)
                return result
            ...
            conn.commit()
            result = self._serialize_application(row, idempotent=False)
        self._retain_application(result)
        return result

    def _retain_application(self, application: dict[str, Any]) -> None:
        if self.retention_ledger is None:
            return
        retained_application = {**application, "idempotent": False}
        self.retention_ledger.append(
            f"paper-corporate-action-{application['application_id']}",
            {
                "schema_version": "open_stock_ai.paper_corporate_action_execution.v1",
                "source_of_truth": "paper_corporate_action_applications",
                "application": retained_application,
            },
            critical=True,
            kind="execution_corporate_action",
            occurred_at=str(application["applied_at"]),
        )
