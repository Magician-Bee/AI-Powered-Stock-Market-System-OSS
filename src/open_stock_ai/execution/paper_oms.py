...
from open_stock_ai.execution.paper_nav import PaperNAVLedger
from open_stock_ai.execution.paper_fill_evidence import build_paper_fill_evidence
...
class PaperOMS:
    ...
    def __post_init__(self) -> None:
        ...
        self.minimum_commission = self._env_float("OPEN_STOCK_AI_PAPER_MINIMUM_COMMISSION", self.minimum_commission, minimum=0.0)
        ...
        if self.retention_ledger is not None:
            ...
            if retention_path is None or Path(retention_path).resolve() != Path(self.store.path).resolve():
                raise ValueError("paper OMS retention must share its durable database")
        self._nav = PaperNAVLedger(self.store, self.account_id, self.nav_timezone)
        self.ensure_account()
    ...
    def submit_partial_fill(
        ...
        *,
        ...
        ``execution_context`` is a Host-only keyword, never read from the order
        payload. Its snapshot is committed with this fill's cash ledger entry.
        """
        ...
        with self.store._connect() as conn:
            ...
            if existing is not None:
                if str(existing["account_id"]) != self.account_id:
                    conn.commit()
                    return self._invalid_result("order_belongs_to_different_account", order_id=order_id)
                ...
                broker_quantity = self._number(ticket.get("requested_quantity"))
                if math.isfinite(broker_quantity) and broker_quantity > 0:
                    quantity, rejection = broker_quantity, None
                    owned = float(position["quantity"] or 0) if position is not None else 0
                    if action in {"sell", "reduce"} and broker_quantity > max(owned, 0):
                        rejection = "insufficient_position"
                    elif action == "buy_to_cover" and broker_quantity > max(-owned, 0):
                        rejection = "insufficient_short_position"
                conn.execute(
                    """
            ...
            commission = self._commission_increment(conn, order_id, gross_amount)
            ...
            borrow_ledger = PaperBorrowLedger(
                self.store, self.account_id, self.retention_ledger
            )
                    ...
                    json.dumps(
                        {
                            ...
                            "fill_evidence": build_paper_fill_evidence(
                                fill={"fill_id": fill_id, "order_id": order_id, "account_id": self.account_id,
                                      "created_at": now, "symbol": symbol, "side": "buy" if is_buy else "sell",
                                      "quantity": quantity, "reference_price": reference_price, "fill_price": fill_price,
                                      "gross_amount": gross_amount, "commission": commission, "tax": tax,
                                      "slippage_cost": slippage_cost, "net_cash_delta": net_cash_delta},
                                market_context=order.get("market_context"), execution_context=execution_context,
                            ),
                        },
                        ensure_ascii=False,
                    ),
    ...
    def portfolio_summary(self, *, as_of: str | datetime | None = None) -> dict[str, Any]:
        with self.store._connect() as conn:
            conn.row_factory = sqlite3.Row
            self._ensure_account(conn)
            conn.commit()
            return self._portfolio_summary(conn, as_of=as_of)
    ...
    def settle_due(self, *, as_of: str | datetime | None = None) -> dict[str, Any]:
        ...
        if settled_ids:
            self._retain_execution(
                "paper-settlement",
                result,
                kind="execution_settlement",
    ...
    def import_corporate_actions(self, payloads: list[dict[str, Any]]) -> dict[str, Any]:
        return CorporateActionLedger(
            self.store, self.retention_ledger
        ).import_actions(payloads)
    ...
    def corporate_actions(
        ...
        *,
        ...
        return CorporateActionLedger(self.store, self.retention_ledger).list_actions(
            symbol=symbol,
            start=start,
            end=end,
            limit=limit,
    ...
    def _ensure_account(self, conn: sqlite3.Connection) -> None:
        ...
        if row is not None:
            return
        now = self._now()
        # An explicit account reset deletes the old account and its fills.
        # Its replacement must not inherit that prior incarnation's NAV peak.
        conn.execute("delete from paper_nav_observations where account_id=?", (self.account_id,))
        conn.execute(
            """
        ...
        self._nav.observe(conn, equity=self.initial_cash,
                          state={"cash_balance": self.initial_cash, "positions": []},
                          as_of=now, inception=True)
    ...
    def _portfolio_summary(self, conn: sqlite3.Connection, *, as_of: str | datetime | None = None) -> dict[str, Any]:
        account = self._account_row(conn)
        rows = conn.execute(
        ...
        borrow_summary = PaperBorrowLedger(
            self.store, self.account_id, self.retention_ledger
        ).summary()
        nav = self._nav.observe(conn, equity=total_equity, state={"cash_balance": cash, "positions": positions},
        ...
        return {
            ...
            "today_pnl": nav["today_pnl"],
            "peak_equity": nav["peak_equity"],
            "current_drawdown_pct": nav["current_drawdown_pct"],
            "nav_risk_state": nav,
    ...
    def _retain_execution(
        ...
        *,
        ...
        self.retention_ledger.append(
            f"{record_prefix}-{sha256(encoded).hexdigest()}",
            payload,
            critical=True,
            kind=kind,
    ...
    def _assumptions(self) -> dict[str, Any]:
        return {
            ...
            "commission_basis": "cumulative_notional_per_order_minimum_charged_once",
    ...
    def _env_float(self, name: str, default: float, *, minimum: float) -> float:
        try:
            value = float(os.getenv(name, str(default)) if self.read_environment else default)
    ...
    def commission_for_notional(self, gross_amount: float, *, order_id: str | None = None) -> float:
        """Preview the remaining commission against already charged fills."""
        if gross_amount <= 0:
            return 0.0
        if order_id is None:
            return max(self.minimum_commission, gross_amount * self.commission_bps / 10_000.0)
        with self.store._connect() as conn:
            return self._commission_increment(conn, order_id, gross_amount)

    def _commission_increment(self, conn: sqlite3.Connection, order_id: str, gross_amount: float) -> float:
        prior_gross, prior_commission = conn.execute(
            "select coalesce(sum(gross_amount),0), coalesce(sum(commission),0) from paper_fills where account_id=? and order_id=?",
            (self.account_id, order_id),
        ).fetchone()
        cumulative = max(self.minimum_commission, (float(prior_gross) + gross_amount) * self.commission_bps / 10_000.0)
        return max(0.0, cumulative - float(prior_commission))
