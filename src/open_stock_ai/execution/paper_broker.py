...
class PaperBrokerSimulator:
    ...
    def preview(self, ticket: dict[str, Any], market: dict[str, Any]) -> dict[str, Any]:
        ...
        market_rules = TaiwanPaperMarketRules().evaluate(
        ...
        commission = gross_amount * self.oms.commission_bps / 10_000.0
    ...
    def get_order(self, order_id: str) -> dict[str, Any]:
        ...
        with self.store._connect() as conn:
            conn.row_factory = sqlite3.Row
            row = conn.execute(
                "select 1 from paper_broker_orders where order_id = ? and account_id = ?",
                (order_id, self.oms.account_id),
            ).fetchone()
            if row is None:
                raise ValueError("paper_broker_order_not_found")
    ...
    def _evaluate_order(self, order_id: str, market: dict[str, Any]) -> dict[str, Any]:
        ...
        with self.store._connect() as conn:
            ...
            marketable, triggered = self._marketability(ticket, market_price, previously_triggered=previously_triggered)
            now = self._now()
            conn.execute(
            ...
            if marketable:
                conn.commit()
                return self._fill(order_id, ticket, market)
    ...
    def _fill(self, order_id: str, ticket: dict[str, Any], market: dict[str, Any]) -> dict[str, Any]:
        account = self.oms.portfolio_summary()
    ...
    def _validate_account_side(
        ...
        *,
        ...
        if side == "sell":
            ...
            estimated = quantity * price * (1.0 + self.oms.commission_bps / 10_000.0)
