...
class ContextBroker:
    ...
    PROFILES = {
        ...
        "market_information": ContextExposureProfile(
            ...
            capability_prefixes=(
                "agent.",
                "market.",
                "autonomy.status", "autonomy.coverage", "autonomy.research", "autonomy.evidence",
        ...
        "paper_execution": ContextExposureProfile(
            ...
            include_market=True,
            include_account=True,
            capability_prefixes=("market.", "paper.", "portfolio.", "autonomy.", "system."),
        ),
    ...
    def filter_capabilities(
        ...
        *,
        ...
        phase: str | None = None,
        autonomous_campaign: bool = False,
    ) -> list[dict[str, Any]]:
        ...
        if autonomous_campaign:
            prefixes = tuple(dict.fromkeys((*prefixes, "market.", "web.", "portfolio.", "risk.", "autonomy.")))
        if not prefixes:
            return []
        filtered = [
            item
        ...
        if "paper_execution" in composed_kinds and not autonomous_campaign:
            ...
            allowed = {
                ...
                "paper.submit_order",
                "system.capabilities",
                "autonomy.status", "autonomy.coverage", "autonomy.research", "autonomy.evidence", "autonomy.activate", "autonomy.manage", "autonomy.propose_plan", "autonomy.close_plan",
            }
    ...
    def disclose_capabilities(
        ...
        *,
        ...
        if autonomous_campaign:
            # Keep old completed schemas in the compiler inventory, but do not
            # offer another account's paper/portfolio tools to the model.
            filtered = [item for item in filtered if not str(item.get("name") or "").startswith(("paper.", "portfolio."))]
            # These tools carry the isolated account/cycle contract. Preserve
            # them before optional overlays when the context budget trims tools.
            priority = {name: rank for rank, name in enumerate((
                "autonomy.status", "autonomy.coverage", "autonomy.research", "autonomy.evidence",
                "autonomy.propose_plan", "autonomy.activate", "autonomy.manage",
                "autonomy.close_plan", "system.capabilities",
            ))}
            filtered.sort(key=lambda item: priority.get(str(item.get("name") or ""), len(priority)))
