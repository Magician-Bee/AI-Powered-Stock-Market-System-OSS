...
from .autonomy_contract import campaign_execution_authorized
    ...
    def build(
        ...
        *,
        ...
        capability_manifest = self.context_broker.filter_capabilities(
            ...
            task_kind=task_kind,
            autonomous_campaign=campaign_execution_authorized(context),
        )
        capability_snapshot = [
            {
                "name": item.get("name"),
        ...
        state = {
            ...
            "account": (
                {"available": False, "exposed": False, "source_tool": "autonomy.status",
                 "scope": "isolated_autonomous_campaign", "requires_current_host_receipt": True}
                if campaign_execution_authorized(context) else self.account_snapshot()
