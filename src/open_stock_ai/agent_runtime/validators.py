...
def _final_summary_numeric_claims_check(
    *,
    ...
    claims = _material_numeric_claims(str(final_summary or ""))
    ...
    unsupported = sorted(
        (claim for claim in claims if not supported_with_display_rounding(claim)),
...
def _period_tokens(value: str) -> set[str]:
    return {
        ...
        for match in _PERIOD_TOKEN.finditer(value)
    }
    ...
    *,
    ...
    covered = {
        ...
        "technical": (
            ...
            and any(
                "technical_features" in result
                or "signal_summary" in result
                or "pipeline_workspace" in result
...
def _criterion_check(
    ...
    *,
    ...
    negative_execution_constraint = any(
        phrase in lowered
        for phrase in (
            "不交易",
            "不要交易",
    ...
    if any(token in lowered for token in ("test", "pytest", "測試", "compile", "編譯", "syntax")):
        ...
        not negative_execution_constraint
        and any(token in lowered for token in ("order", "委託", "持倉", "交易"))
    ):
        required_prefixes = ("paper.", "portfolio.")
    type_matched = (
        not required_prefixes
        or any(
            str(item.get("tool") or "").startswith(required_prefixes)
