    ...
    def handle_trigger(
        ...
        *,
        ...
        if automation["state"] != AutomationState.ACTIVE.value:
            raise ValueError("only active automations can handle triggers")
        ...
        existing_execution = (
            self.store.get_execution_by_source_event(automation_id, source_event_id)
            if source_event_id
            else None
        )
        if existing_execution and existing_execution.get("completed_at"):
            return _existing_execution_outcome(automation_id, existing_execution)
        ...
        candidate = (cheap_filter or _cheap_filter)(event, intent.decision_logic)
        if not candidate:
            self.store.complete_execution(
                execution_id,
                stage="cheap_filter",
                status="filtered",
                output_payload={"condition_candidate": False},
            ...
            return TriggerOutcome(
                ...
                None,
                None,
                "cheap deterministic filter rejected the event",
                execution_id,
            )
        self.store.checkpoint_execution(
            ...
            stage="cheap_filter",
            status="candidate",
            output_payload={"condition_candidate": True},
            now=current,
        )
        self._transition(automation_id, AutomationState.TRIGGERED, now=current)
        previous = automation.get("current_decision") or None
        try:
    ...
    async def handle_trigger_async(
        ...
        *,
        ...
        if not candidate:
            ...
            return TriggerOutcome(
                automation_id, False, False, False, False, None, None,
                "cheap deterministic filter rejected the event", execution_id,
            )
        self.store.checkpoint_execution(
            execution_id,
            stage="cheap_filter",
...
def _decision_changed(previous: Mapping[str, Any] | None, current: Mapping[str, Any]) -> bool:
    ...
    after = {key: current.get(key) for key in keys if key in current}
    if before != after:
        return True
    old_confidence = float(previous.get("confidence") or 0.0)
    new_confidence = float(current.get("confidence") or 0.0)
    return abs(new_confidence - old_confidence) >= 0.15
