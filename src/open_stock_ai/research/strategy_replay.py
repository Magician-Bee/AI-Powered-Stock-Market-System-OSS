...
from open_stock_ai.strategy.execution_policy import plan_signal_order, policy_metadata, protective_exit
...
class ExactStrategyReplay:
    ...
    historical_impact_sensitivity_bps: float | None = None
    # Minimum adverse move on each buy AND sell, independent of historical
    # calibration dates. Scenario input only; never broker execution proof.
    adverse_execution_floor_bps: float | None = None
    ...
    def __post_init__(self) -> None:
        value = self.adverse_execution_floor_bps
        if value is not None and (isinstance(value, bool) or not isinstance(value, (int, float))
                                  or not isfinite(value) or not 0 <= value < 10000):
            raise ValueError("invalid_adverse_execution_floor_bps")
    ...
    def evaluate(
        ...
        return {
            ...
            "strategy_version_manifest": self.strategy_version_manifest(),
            "execution_policy": policy_metadata(),
            "central_risk_engine_replayed": False,
    ...
    def strategy_version_hash(self) -> str:
        return content_hash(self.strategy_version_manifest())

    def strategy_version_manifest(self) -> dict[str, Any]:
        manifest = strategy_source_manifest(self.strategy)
        research_root = Path(__file__).resolve().parent
        manifest["replay_sources"] = {
            name: hashlib.sha256((research_root / name).read_bytes()).hexdigest()
            for name in ("strategy_replay.py", "cost_model.py")
        }
        manifest["replay_sources"]["execution/taiwan_market_rules.py"] = hashlib.sha256(
            (research_root.parent / "execution/taiwan_market_rules.py").read_bytes()).hexdigest()
        manifest["execution_policy"] = policy_metadata()
        manifest["replay_configuration"] = {
            name: getattr(self, name) for name in (
                "initial_cash", "latency_bars", "feature_lookback", "label_horizon_bars",
                "embargo_bars", "max_participation_rate", "transaction_cost_bps",
                "slippage_bps", "exchange_fee_bps", "impact_coefficient_bps", "min_points",
                "historical_impact_sensitivity_bps", "adverse_execution_floor_bps",
            )
        }
        manifest["cost_model"] = self._cost_model_receipt()
        return manifest
    ...
    def _replay_slice(
        ...
        *,
        ...
        def process_order(order: dict[str, Any], row: dict[str, Any]) -> dict[str, Any] | None:
            nonlocal cash, position_quantity, trade_count, remaining_capacity
            receipt = {
            ...
            fill = self._fill_pending_order(
                order=order,
                row={**row, "_remaining_execution_capacity": remaining_capacity},
                cash=cash,
            ...
            trade_count += 1
            remaining_capacity -= int(fill["quantity"])
            if fill["side"] == "buy":
                entry_lots.append({"quantity": fill["quantity"], "order": order})
            else:
                remaining = fill["quantity"]
                ordered_lots = entry_lots
                if order.get("exit_entry_order_id"):
                    ordered_lots = sorted(entry_lots, key=lambda item: item["order"]["order_id"] != order["exit_entry_order_id"])
        ...
        for index in range(start, replay_stop):
            row = rows[index]
            remaining_capacity = int(floor(row["volume"] * self._participation_rate()))
            ...
            for order in due:
                process_order(order, row)

            # Brackets were fixed by earlier entry decisions. They apply only
            # to filled shares, including entry-open fills in this bar. Per-lot
            # levels prevent an add from loosening a previous entry's stop.
            for lot in list(entry_lots):
                if lot["quantity"] <= 0:
                    continue
                entry = lot["order"]
                if lot.get("pending_exit"):
                    trigger = {**lot["pending_exit"], "reference_price": float(row["open"]),
                               "residual_market_exit": True, "ambiguous_bar": False}
                else:
                    trigger = protective_exit(
                        open_price=float(row["open"]), high=float(row["high"]), low=float(row["low"]),
                        stop_loss=entry.get("stop_loss"), target_price=entry.get("target_price"),
                    )
                if trigger is None:
                    continue
                # Once triggered, an unfilled protective exit cannot silently
                # revert to an armed stop when the following bar rebounds.
                lot["pending_exit"] = trigger
                order_id = f"replay-protective-{index}-{entry['order_id']}"
                decision_id = f"{order_id}-decision"
                public = {
                    "order_id": order_id, "decision_id": decision_id,
                    "submitted_at": entry["order_time"], "scheduled_broker_receive_time": row["timestamp"],
                    "signal_action": "sell", "target_position": 0,
                    "status": "pending_broker_receipt", "fill_ids": [],
                    "protective_exit": trigger, "entry_order_id": entry["order_id"],
                }
                decision = {
                    "decision_id": decision_id, "order_id": order_id,
                    "feature_available_time": entry["feature_available_time"],
                    "decision_time": entry["decision_time"], "order_time": entry["order_time"],
                    "broker_receive_time": None, "fill_time": None,
                    "signal_time": entry["decision_time"], "execution_time": row["timestamp"],
                    "action": "sell", "confidence": None, "target_position": 0,
                    "position_quantity_at_decision": lot["quantity"],
                    "broker_receipt_id": None, "fill_ids": [], "status": "pending_broker_receipt",
                    "protective_exit": trigger,
                }
                protective_order = {
                    **entry, "order_id": order_id, "decision_id": decision_id,
                    "fill_index": index, "side": "sell", "requested_quantity": lot["quantity"],
                    "lot_size": 1000 if request.market == "TW" and lot["quantity"] >= 1000 else 1,
                    "lot_type": "board_lot" if request.market == "TW" and lot["quantity"] >= 1000 else "odd_lot",
                    "reference_price_override": trigger["reference_price"],
                    "exit_entry_order_id": entry["order_id"], "protective_exit": trigger,
                    "audit_entry": decision, "public_record": public,
                }
                orders.append(public)
                audit.append(decision)
                event_ledger.append({"event_id": order_id, "event_type": "protective_order_triggered",
                                     "occurred_at": row["timestamp"], "order_id": order_id,
                                     "entry_order_id": entry["order_id"], **trigger})
                process_order(protective_order, row)
            entry_lots = [lot for lot in entry_lots if lot["quantity"] > 0]
            ...
            target_position = position_quantity
            if index + self.latency_bars < replay_stop:
                ...
                snapshot = MarketSnapshot(
                    ...
                    raw={
                        ...
                        "position_quantity": position_quantity,
                        "equity": cash + position_quantity * row["close"],
                        "holding_bars": (
                            index - min(lot["order"]["fill_index"] for lot in entry_lots)
                            if entry_lots else 0
                        ),
                    },
                )
                signal = active_strategy.generate_signal(request, snapshot, intelligence)
                fee_quote = self._cost_quote(order=row, row=row, side="buy")
                execution_plan = plan_signal_order(
                    signal, equity=cash + position_quantity * row["close"], cash=cash,
                    reference_price=row["close"], position_quantity=position_quantity,
                    commission_bps=fee_quote.commission_bps, exchange_fee_bps=fee_quote.exchange_fee_bps,
                    minimum_commission=fee_quote.commission_minimum_twd,
                )
                if signal.action in {"buy", "add"} and float(signal.confidence or 0.0) < min_confidence:
                    execution_plan = {**execution_plan, "eligible": False, "quantity": 0,
                                      "reason": "frozen_confidence_filter"}
                signed = execution_plan["quantity"] * (1 if execution_plan["side"] == "buy" else -1)
                target_position = position_quantity + signed
                ...
                pending_orders.append(
                    {
                        ...
                        "requested_quantity": execution_plan["quantity"],
                        "cash_budget": execution_plan["cash_budget"],
                        "stop_loss": signal.stop_loss,
                        "target_price": signal.target_price,
                        "venue": row["venue"],
                        "product_type": row["product_type"],
                        "lot_type": execution_plan.get("lot_type", row["lot_type"]),
    ...
    def _fit_policy(
        ...
        *,
        ...
        # The execution policy is fixed. Do not silently optimize a second
        # strategy using gross directional labels that omit sizing and costs.
        candidates = (0.0,)
        ...
        payload = {
            ...
            "selection_method": "fixed_zero_confidence_filter_no_optimization",
            "score_metric": "training_directional_label_diagnostic_not_policy_pnl",
    ...
    def _fill_pending_order(
        ...
        *,
        ...
        account_cash = cash
        reference_price = float(order.get("reference_price_override", row["open"]))
        side = order.get("side")
        requested_quantity = max(0, int(order.get("requested_quantity") or 0))
        if side not in {"buy", "sell"} or requested_quantity == 0:
            return None
        if side == "buy":
            # An entry whose gap invalidates its already-declared bracket is
            # not silently opened with an immediately violated risk bound.
            if reference_price <= float(order.get("stop_loss") or 0):
                return None
            if order.get("target_price") is not None and reference_price >= float(order["target_price"]):
                return None
            cash = min(cash, float(order["cash_budget"]))
        ...
        capacity = min(capacity, int(row.get("_remaining_execution_capacity", capacity)))
        lot_size = int(order.get("lot_size") or 1)
        quantity = min(requested_quantity, capacity) // lot_size * lot_size
        if quantity <= 0:
            return None
        ...
        if side == "buy":
            ...
            quantity = min(quantity, max(0, affordable)) // lot_size * lot_size
            if quantity <= 0:
                return None
            fill_price, cost_receipt = self._fill_price(
                reference_price=reference_price,
        ...
        return {
            ...
            "order_id": order["order_id"],
            "protective_exit": order.get("protective_exit"),
            "side": side,
            "quantity": quantity,
            "signed_quantity": signed_quantity,
            "requested_quantity": requested_quantity,
            "partial_fill": quantity < requested_quantity,
            "unfilled_remainder_cancelled_at_bar_end": quantity < requested_quantity,
            "reference_price": reference_price,
        ...
        *,
        ...
        try:
            quote = self.impact_schedule.quote(
                venue=str(row["venue"]), product_type=str(row["product_type"]), side=side,
                trade_at=str(row["timestamp"]), **inputs,
            )
        except UnsupportedCostSchedule as exc:
            if self.historical_impact_sensitivity_bps is None or not str(exc).startswith("impact_rule_unverified:"):
                raise
            # Pre-2026 history is outside the installed calibrated-date scope.
            # An explicit research sensitivity is allowed, but cannot be
            # mislabeled as a venue/date-calibrated execution rule.
            quote = PointInTimeMarketImpactModel(
                impact_coefficient_bps=self.historical_impact_sensitivity_bps,
            ).quote(side=side, **inputs)
            quote = replace(quote, calibration_status="historical_sensitivity_assumption",
        ...
        fill_price, receipt = quote.fill_price(), quote.receipt()
        if self.adverse_execution_floor_bps is not None:
            # Preserve larger data-driven impact; the floor only makes the
            # scenario more adverse. Linear sell reduction matches PaperOMS's
            # proxy instead of the slightly cheaper reciprocal formulation.
            direction = 1 if side == "buy" else -1
            floor_price = Decimal(str(reference_price)) * (
                Decimal(1) + direction*Decimal(str(self.adverse_execution_floor_bps))/Decimal(10000))
            floor_price = max(Decimal(str(fill_price)), floor_price) if side == "buy" else min(Decimal(str(fill_price)), floor_price)
            tick_policy = "not_applied_to_non_equity_product"
            if row.get("venue") in {"TWSE", "TPEX"} and row.get("product_type") == "stock":
                from open_stock_ai.execution.taiwan_market_rules import tick_size
                tick = tick_size(floor_price)
                floor_price = (floor_price/tick).to_integral_value(rounding=ROUND_CEILING if side == "buy" else ROUND_FLOOR)*tick
                tick_policy = "shared_taiwan_equity_tick_adverse_rounding"
            if floor_price <= 0:
                raise UnsupportedCostSchedule("adverse_floor_has_no_positive_executable_price_tick")
            fill_price = float(floor_price)
            receipt.update(
                underlying_total_slippage_bps=receipt["total_slippage_bps"],
                underlying_calibration_status=receipt["calibration_status"],
                adverse_execution_floor_bps=self.adverse_execution_floor_bps,
                adverse_floor_tick_policy=tick_policy,
                total_slippage_bps=direction*(fill_price/reference_price-1)*10000,
                calibration_status="execution_scenario_adverse_floor",
                calibration_execution_evidence_eligible=False, execution_evidence_eligible=False,
            )
        return fill_price, receipt
    ...
    def _cost_model_receipt(self) -> dict[str, Any]:
        return {
            ...
            "historical_impact_sensitivity_bps": self.historical_impact_sensitivity_bps,
            "adverse_execution_floor_bps": self.adverse_execution_floor_bps,
            "adverse_floor_scope": "each_entry_and_exit_including_dated_and_historical_sensitivity_rules",
