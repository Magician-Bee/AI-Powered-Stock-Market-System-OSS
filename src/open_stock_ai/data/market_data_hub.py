...
class MarketDataHub:
    ...
    def load(self, request: StockRequest) -> MarketSnapshot:
        ...
        latest_price = summary_payload.get("latest_price") or {}
        if isinstance(latest_price, dict) and latest_price.get("date"):
            price_timestamp = latest_price["date"]
        elif summary_payload.get("quote_kind") == "last_trade":
            # The legacy gateway's bundle timestamp can be reception time.
            # A last trade needs its own exchange clock; never substitute it.
        ...
        source_contract = quote_envelope(
            ...
            trading_state=summary_payload.get("trading_state", "unknown"),
        )
        horizon_gate = execution_eligibility(source_contract, horizon=request.horizon)
        blockers.extend(horizon_gate["blockers"])
        decision_ready = (
            not blockers and source_policy.get("overall_allowed") is True
            and horizon_gate["execution_eligible"] is True
        )
        analysis_ready = bool(
            price is not None
        ...
        raw["data_contract"] = {
            ...
            "exchange_timestamp": price_timestamp,
            "is_realtime": horizon_gate["current_last_trade"],
