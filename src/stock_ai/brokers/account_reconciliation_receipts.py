...
class BrokerAccountReconciliationReceiptStore:
    def __init__(
        ...
        *,
        ...
        if require_critical_retention and retention_ledger is None:
            raise ValueError("account reconciliation requires a critical retention ledger")
        if retention_ledger is not None:
            retention_path = getattr(retention_ledger.store, "path", None)
            if retention_path is None or Path(retention_path).resolve() != self.path:
                raise ValueError("account reconciliation retention must share its durable database")
        self._initialize()
    ...
    def record(self, receipt: BrokerAccountReconciliationReceipt) -> BrokerAccountReconciliationReceipt:
        ...
        self._retain(
            ...
            receipt.model_dump(),
            kind="execution_account_reconciliation",
            occurred_at=_utc(receipt.observed_at).isoformat(),
        )
        return receipt
    ...
    def record_execution(
        ...
        self._retain(
            receipt.receipt_sha256,
            receipt.model_dump(),
            kind="execution_account_reconciliation_trigger",
            occurred_at=_utc(receipt.observed_at).isoformat(),
        )
        return receipt
