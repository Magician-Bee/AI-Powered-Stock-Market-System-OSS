...
from open_stock_ai.governance.security_audit import SecurityTradingAuditTrail
...
class BrokerOrderManagementGateway:
    ...
    async def submit(
        ...
        *,
        ...
        if datetime.now(timezone.utc) - checked_at > timedelta(seconds=30):
            self._record_pretrade_security_decision(
                entry,
                risk_decision,
                outcome="blocked",
                reason="host risk approval expired before submission",
            )
        ...
        if risk_decision.approved is not True:
            self._record_pretrade_security_decision(
                ...
                risk_decision,
                outcome="blocked",
                reason="central broker risk gate rejected the order",
            )
        ...
        self._record_pretrade_security_decision(
            ...
            risk_decision,
            outcome="allowed",
            reason="central broker risk gate approved the order",
        )
    ...
    def _record_pretrade_security_decision(
        self,
        entry: OrderLedgerEntry,
        decision: BrokerRiskDecision,
        *,
        outcome: str,
        reason: str,
    ) -> None:
        """Bind the Host risk decision to the exact intent before broker I/O."""

        if self.retention_ledger is None or self.retention_ledger.store is None:
            return
        intent_payload = entry.intent.model_dump(mode="json")
        ...
        event_id = hashlib.sha256(
            f"{entry.intent.intent_id}:{decision_sha256}:{outcome}:{reason}".encode("utf-8")
        ).hexdigest()
        SecurityTradingAuditTrail(self.retention_ledger).record(
            event_id=f"broker-risk-{event_id}",
            category="pretrade_risk",
            actor_id="host-central-risk-gate",
            subject_type="broker_order",
            subject_id=entry.intent.intent_id,
            decision="approve_pretrade" if outcome == "allowed" else "reject_pretrade",
            outcome=outcome,
            reason=reason,
            ...
            details={
                ...
                "environment": entry.intent.environment,
                "checks": dict(decision.checks),
                "reasons": list(decision.reasons),
            },
            occurred_at=decision.checked_at.isoformat(),
        )
