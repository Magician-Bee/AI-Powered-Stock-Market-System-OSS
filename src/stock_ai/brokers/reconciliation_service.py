    ...
    def __init__(
        ...
        *,
        ...
        if require_critical_retention and retention_ledger is None:
            raise ValueError("broker reconciliation requires a critical retention ledger")
        if retention_ledger is not None:
            ...
            if retention_path is None or Path(retention_path).resolve() != self.path:
                raise ValueError("broker reconciliation retention must share its durable database")
    ...
    def record(self, receipt: ReconciliationExecutionReceipt) -> ReconciliationExecutionReceipt:
        ...
        if self.retention_ledger is not None:
            self.retention_ledger.append(
                f"broker-reconciliation-{receipt.receipt_sha256}",
                receipt.as_dict(),
                critical=True,
