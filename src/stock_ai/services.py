...
from .phase1_data import list_institutional_flows, list_monthly_revenues, list_securities_master
...
def search_entities(q: str = ""):
    ...
    # Search the persisted registry first so identity-only and pre-listing
    # catalogue rows remain discoverable without needing a quote endpoint.
    # A live source fallback can still discover a row not yet synchronized.
    try:
        stored = list_securities_master(
            q=query, market="taiwan", limit=30, include_lifecycle=True,
        )
        results.extend(Entity(
            entity_id=item.entity_id or entity_id_for_symbol(
                item.symbol, market=item.market, exchange=item.exchange,
            ),
            symbol=item.symbol,
            name=item.name,
            ...
            currency="TWD",
            sector=item.product_classification.get("product_type") or None,
            industry=item.industry,
            is_active=item.lifecycle_status == "active",
            lifecycle_status=item.lifecycle_status,
        ) for item in stored)
    except Exception as exc:
        logger.debug("Stored Taiwan entity search unavailable: %s", exc, exc_info=True)
...
def get_market_summary(symbol: str, *, include_events: bool = True) -> MarketSummary | None:
    ...
    if is_taiwan_code(symbol):
        try:
            ...
            current_last_trade = (
                has_last_trade
                and trading_state in {"trading", "closing_auction", "delayed_close"}
                and rt.get("is_stale") is not True
            )
            ...
            if display_price is None:
                ...
                return None
            latest = PricePoint(
                date=(rt.get("exchange_timestamp") or f"{rt.get('date', '')} {rt.get('time', '')}".strip()).replace("T", " ", 1),
            ...
            summary = MarketSummary(
                ...
                trend=f"{'intraday_realtime' if current_last_trade else trading_state}_{price_basis}",
                fundamentals=None,
                flow=None,
                ...
                freshness_note=(
                    f"公開網頁行情，更新間隔約 {int((rt.get('user_delay_ms') or 5000)/1000)} 秒；"
                    f"市場狀態={trading_state}；價格基準={price_basis}；資料新鮮度以交易所時間判定，接收時間不代表新成交。"
                ...
                source_trade={
                    "provider_id": "twse_mis", "symbol": display_symbol, "venue": exchange,
                    "channel": rt.get("market_session"), "exchange_timestamp": rt.get("exchange_timestamp"),
                    "trade_id": str(rt.get("sequence")) if rt.get("sequence") is not None else None,
                    "price": rt.get("last_price"), "size_shares": rt.get("last_trade_size_shares"),
...
def get_execution_price_summary(symbol: str) -> MarketSummary | None:
    ...
    if detail is not None:
        if detail.quote_kind == "official_close":
            return detail
        if detail.quote_kind == "last_trade" and detail.realtime:
            return detail
    if not is_taiwan_code(symbol):
        return detail
    try:
