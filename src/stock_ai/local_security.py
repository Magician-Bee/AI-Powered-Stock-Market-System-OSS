...
_SESSION_PATH_PREFIXES = ("/api", "/agent/autonomy")


def requires_runtime_session(path: str) -> bool:
    """Cover authenticated route families independently of their mount prefix."""

    return any(path == prefix or path.startswith(prefix + "/") for prefix in _SESSION_PATH_PREFIXES)


def _is_test_transport(request: Request) -> bool:
    # Only synthetic ASGI transports have this server address. A normal
    # connection's transport address must never be inferred from its Host.
    server = request.scope.get("server")
    return request.url.hostname == "testserver" and bool(server) and server[0] == "testserver"
