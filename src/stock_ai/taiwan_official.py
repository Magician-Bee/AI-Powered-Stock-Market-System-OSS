...
_MAX_OFFICIAL_JSON_BYTES = 64 * 1024 * 1024

_CACHE_LOCK = RLock()
_CACHE: dict[tuple[Any, ...], tuple[float, Any]] = {}
_CACHE_LOADED_AT: dict[str, str] = {}
_CACHE_IN_FLIGHT: dict[tuple[Any, ...], Future[Any]] = {}
_CACHE_GENERATION = 0


def _cached(key: tuple[Any, ...], ttl_seconds: int, loader, *, cache_empty: bool = True):
    """Coalesce concurrent misses before transport admission; never cache errors.

    Waiters share this attempt's result or exception, including uncached empty
    history. Different keys still load independently under the existing guard.
    """
    with _CACHE_LOCK:
        cached = _CACHE.get(key)
        if cached and cached[0] > time.monotonic():
            return cached[1]
        pending = _CACHE_IN_FLIGHT.get(key)
        owner = pending is None
        if owner:
            pending = Future()
            _CACHE_IN_FLIGHT[key] = pending
    ...
    try:
        ...
        # Interruptions must also release followers; the original error is
        # propagated unchanged and a later call can make a fresh attempt.
...
def clear_official_caches() -> None:
    ...
    with _CACHE_LOCK:
        ...
        # Keep sharing an active transport, but do not let an older attempt
        # repopulate a cache that was explicitly invalidated while it ran.
        _CACHE_GENERATION += 1
...
def _read_official_json_bytes(url: str, *, timeout: int) -> bytes:
    headers = {
    ...
    with urlopen(Request(url, headers=headers), timeout=timeout) as response:
        ...
        try:
            declared_length = int(response_headers.get("Content-Length") or 0)
        except (TypeError, ValueError):
            declared_length = 0
        if declared_length > _MAX_OFFICIAL_JSON_BYTES:
            raise ValueError("official_json_response_oversize")
        try:
            complete = response.read()
            if len(complete) > _MAX_OFFICIAL_JSON_BYTES:
                raise ValueError("official_json_response_oversize")
            return complete
        except IncompleteRead as incomplete:
            partial = bytes(incomplete.partial)
            try:
                total = int(response.headers.get("Content-Length") or 0)
            except (TypeError, ValueError):
                raise incomplete from None
            if (
                response_headers.get("Accept-Ranges", "").casefold() != "bytes"
                or total <= len(partial)
                or total > _MAX_OFFICIAL_JSON_BYTES
                or len(partial) + int(incomplete.expected) != total
            ):
                raise incomplete from None
    ...
    while len(body) < total:
        start = len(body)
        end = min(total - 1, start + _RANGE_CHUNK_BYTES - 1)
        range_headers = {**headers, "Range": f"bytes={start}-{end}", "If-Range": validator}
        with urlopen(Request(url, headers=range_headers), timeout=timeout) as response:
            status_value = getattr(response, "status", None)
            status = int(status_value if status_value is not None else response.getcode())
            match = _CONTENT_RANGE.fullmatch(str(response.headers.get("Content-Range") or ""))
            if (
                status != 206
                or match is None
                or tuple(map(int, match.groups())) != (start, end, total)
                or (etag is not None and response.headers.get("ETag") != etag)
                or (modified is not None and response.headers.get("Last-Modified") != modified)
            ):
                raise ValueError("official_json_range_response_mismatch")
            chunk = response.read()
        if len(chunk) != end - start + 1:
            raise ValueError("official_json_range_body_length_mismatch")
        body.extend(chunk)
    if len(body) != total:
        raise ValueError("official_json_reassembled_length_mismatch")
    return bytes(body)
...
def _get_json(url: str, timeout: int = 20) -> Any:
    ...
    def load_with_retries() -> Any:
        ...
        for attempt in range(attempts):
            try:
                return json.loads(_read_official_json_bytes(url, timeout=effective_timeout).decode("utf-8-sig"))
...
def tpex_indices() -> list[dict[str, Any]]:
    ...
    def load() -> list[dict[str, Any]]:
        ...
        # Every path belongs to the same registered tpex_index dataset scope.
        # Read it sequentially and honor the conservative one-second interval
        # used when the provider has not published a verified rate policy.
        results = []
        for index, definition in enumerate(TPEX_INDEX_SERIES):
            if index:
                time.sleep(1.0)
            results.append(fetch(definition))
        return results
...
def search_taiwan_official(q: str, limit: int = 30) -> list[Entity]:
    ...
    # Search is a discovery surface across independent official venues.  A
    # transient TPEx failure must not erase an already verified TWSE match (or
    # vice versa).  Whole-universe research records source coverage separately;
    # this function only returns the valid matches it actually obtained.
    def available(loader) -> list[dict[str, Any]]:
        try:
            rows = loader()
        except Exception:
            return []
    ...
    for row in available(twse_quotes):
        code = str(row.get("Code", "")).strip()
    ...
    for row in available(tpex_quotes):
        code = str(row.get("SecuritiesCompanyCode", "")).strip()
    ...
    for row in available(twse_companies):
    ...
    for row in available(tpex_companies):
        code = str(row.get("SecuritiesCompanyCode", "")).strip()
