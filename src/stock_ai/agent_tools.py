...
from .tool_providers.autonomy import AutonomousTradingToolProvider
...
class StockAgentToolRegistry:
    ...
    def __init__(self) -> None:
        ...
        self.autonomy_tools = AutonomousTradingToolProvider()
        self._specs = {
            spec.name: spec
            for spec in (
                AgentToolSpec(
    ...
    async def _execute_core(
        ...
        if name == "paper.preview_order":
            ...
            context.state.setdefault("paper_previews", {})[signature] = {
                ...
                "market_signature": _paper_market_signature(result.get("market") or {}),
        ...
        if name == "paper.submit_order":
            ...
            market_changed = (
                _paper_market_signature(current_market) != receipt["market_signature"]
                if receipt.get("market_signature") else
            ...
            if has_local_preview and market_changed:
...
def _paper_market_signature(market: dict[str, Any]) -> str:
    """Bind preview to execution facts, allowing the same quote to be fetched again.

    The Host preview revalidates the signed envelope and current freshness on
    every submission. Reception time and calculated age are not a new trade;
    exchange time, eligibility, liquidity and trading state still bind it.
    """
    source = dict(market.get("source_envelope") or {})
    if source.get("provider_id") and source.get("exchange_timestamp"):
        source = {key: value for key, value in source.items() if key not in {"received_at", "signature"}}
    payload = {
        key: market.get(key) for key in (
            "symbol", "market", "exchange", "price", "source_kind", "source_timestamp",
            "is_realtime", "is_fallback", "limit_up", "limit_down", "trading_state",
            "buy_liquidity_confirmed", "sell_liquidity_confirmed",
            "exchange_rules_enforced", "settlement_rules_enforced",
        )
    }
    payload["source_envelope"] = source
    payload["execution_eligibility"] = {
        key: value for key, value in dict(market.get("execution_eligibility") or {}).items()
        if key not in {"received_at", "age_seconds"}
    }
