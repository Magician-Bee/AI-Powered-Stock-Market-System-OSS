...
def _verified_price(
    ...
    *,
    ...
    is_realtime = eligibility["current_last_trade"]
    non_realtime_kind = (
        "closed_last_trade" if "last_trade_market_not_open" in eligibility["blockers"]
        else "stale_last_trade" if "quote_expired" in eligibility["blockers"]
        else "delayed_last_trade"
    )
    source_kind = {
        "last_trade": "realtime_last_trade" if is_realtime else non_realtime_kind,
    ...
    result = {
        ...
        "source_timestamp": eligibility["exchange_timestamp"] or summary.latest_price.date,
        ...
        "source_trade": getattr(summary, "source_trade", None),
...
def paper_training_mark_to_market() -> dict[str, Any]:
    ...
    for symbol in symbols:
        try:
            price = _verified_price(symbol, require_execution_quote=False)
            if symbol in position_symbols:
                marks.append(
                    lab.apply_market_mark(
                        ...
                        is_fallback=price["is_fallback"],
                        metadata={
                            "source_kind": price["source_kind"], "reason": "portfolio_mark_to_market",
                            "source_envelope": price["source_envelope"],
                            "execution_eligibility": price["execution_eligibility"],
                        },
                    )
                )
            if price["execution_eligibility"]["execution_eligible"] is True:
                tick_result = broker.process_market_tick(price["symbol"], price)
                order_updates.extend(tick_result.get("results") or [])
            else:
                execution_skips.append({"symbol": symbol, **price["execution_eligibility"]})
