...
def load_all_taiwan_features() -> list[dict[str, Any]]:
    """Load one deterministic feature row for every active TWSE/TPEx instrument.
    ...
    the official security master in memory.
    """
    ...
    for row in twse_quotes():
        ...
        if code:
            quote_rows[f"{code}.TW"] = ("TWSE_ALL_QUOTES", row)
    for row in tpex_quotes():
        code = str(row.get("SecuritiesCompanyCode") or "").strip()
        if code:
            quote_rows[f"{code}.TWO"] = ("TPEX_DAILY_QUOTES", row)
    ...
    for symbol, item in sorted(master.items()):
        source, row = quote_rows.get(symbol, ("OFFICIAL_SECURITY_MASTER", {}))
        ...
        features.append(
            _feature(
                symbol=symbol,
                entity_id=getattr(item, "entity_id", None) or symbol,
                name=item.name,
                exchange=item.exchange,
                industry=item.industry,
                is_etf=item.is_etf,
                is_warrant=item.is_warrant,
    ...
    return attach_current_source_inputs(
        features,
        revenues=revenues,
