...
def _market_candidate(
    ...
    *,
    ...
    if feature.get("is_warrant") or trade_value < 10_000_000 or abs(change) >= 8:
        return "high_risk", {
            "label": "高風險",
            "reason": "流動性、商品類型或單日波動觸發 Host 高風險門檻。",
    ...
    *,
    ...
    for feature in features:
        ...
        if symbol in positions:
            ...
            if (
                action == "add"
                and str((feature.get("data_quality") or {}).get("status") or "") != "ready"
            ):
                action, label, reason = (
                    "hold",
                    "持有／待驗證",
                    "主要報價尚未完成獨立同日交叉驗證；暫停加碼，保留既有部位監控。",
                )
            category = action
            explanation = {
                "label": label,
