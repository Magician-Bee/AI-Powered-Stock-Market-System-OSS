...
from .product_projection import product_assessment, product_counts
...
def _candidate_view(item) -> dict[str, Any]:
    """Reassess legacy saved rows locally; never rewrite the saved snapshot."""
    view = item.model_dump(mode="json")
    assessment = product_assessment(view)
    view["product_entry_assessment"] = assessment
    if not assessment["allowed"]:
        view["host_risk_status"] = "blocked"
        if view.get("is_position"):
            if view.get("portfolio_action") == "add":
                view.update(portfolio_action="hold", category="hold", market_category="AVOID_NOW",
                            decision_label="持有／商品資格待確認")
        else:
            view.update(category="avoid_now" if assessment["classification_verified"] else "insufficient_data",
                        market_category="AVOID_NOW" if assessment["classification_verified"] else "INSUFFICIENT_DATA",
                        decision_label="研究保留／新進場受限")
        view["product_entry_reasons"] = assessment["reasons"]
    return view


def _portfolio_views(snapshot) -> dict[str, list[dict[str, Any]]]:
    result = {action: [] for action in ("hold", "add", "reduce", "exit")}
    if snapshot:
        for item in snapshot.candidate_details.values():
            view = _candidate_view(item)
            if view.get("is_position"):
                result[view.get("portfolio_action") or "hold"].append(view)
    return result


def _bootstrap_snapshot(snapshot, *, compact: bool = True) -> dict[str, Any] | None:
    if snapshot is None:
        return None
    views = {symbol: _candidate_view(item) for symbol, item in snapshot.candidate_details.items()}
    rankings = {category: [] for category in snapshot.rankings}
    portfolio = {action: [] for action in ("hold", "add", "reduce", "exit")}
    for symbol, view in views.items():
        if view.get("is_position"):
            portfolio[view.get("portfolio_action") or "hold"].append(symbol)
        else:
            rankings.setdefault(view["category"], []).append(symbol)
    selected_symbols: list[str] = []
    compact_rankings: dict[str, list[str]] = {}
    for category, symbols in rankings.items():
        ordered = sorted(symbols, key=lambda symbol: (views[symbol]["rank"], symbol))
        compact_rankings[category] = ordered[:30] if compact else ordered
        selected_symbols.extend(compact_rankings[category])
    for symbols in portfolio.values():
        selected_symbols.extend(symbols)
    selected_symbols = list(dict.fromkeys(selected_symbols))
    classification_counts = {
        category: sum(1 for item in views.values() if not item["is_position"] and item["market_category"] == category)
        for category in ("BUY_NOW", "WATCH", "FUTURE_BUY", "AVOID_NOW", "INSUFFICIENT_DATA")
    }
    portfolio_counts = {
        action: len(portfolio.get(action, []))
        for action in ("hold", "add", "reduce", "exit")
    }
    universe = snapshot.universe.model_dump(mode="json")
    # Reassess the saved receipts at read time. Legacy flags and zero values
    # cannot substitute for missing product evidence; storage stays immutable.
    universe.update(product_counts(list(views.values())))
    return {
...
def _full_snapshot_view(snapshot) -> dict[str, Any]:
    return {**snapshot.model_dump(mode="json"), **_bootstrap_snapshot(snapshot, compact=False)}


def _compact_candidates(snapshot, category: str, limit: int = 30) -> list[dict[str, Any]]:
    if snapshot is None:
        return []
    items = [_candidate_view(item) for item in snapshot.candidate_details.values()]
    return sorted((item for item in items if not item["is_position"] and item["category"] == category),
                  key=lambda item: (item["rank"], item["symbol"]))[:limit]
...
def _market_rankings(snapshot) -> dict[str, list[dict[str, Any]]]:
    ...
    details = list(snapshot.candidate_details.values())
    executable = [
        item
        for item in details
    ...
    return {
        "volume": [_candidate_view(item) for item in volume],
        "movers": [_candidate_view(item) for item in movers],
        "industries": industries[:30],
        "anomalies": [_candidate_view(item) for item in anomalies],
    }
...
def _market_decision_lists(snapshot) -> dict[str, list[dict[str, Any]]]:
    ...
    details = [_candidate_view(item) for item in snapshot.candidate_details.values()]
    by_category = {
        key: [item for item in details if not item["is_position"] and item["market_category"] == category]
        for key, category in (
    ...
    by_category["sell_reduce"] = [
        item for item in details if item["is_position"] and item["portfolio_action"] in {"reduce", "exit"}
    ]
...
def _is_excluded_product(item: Any) -> bool:
    """Product exclusion applies only to new entry, not research navigation."""
    return not product_assessment(item)["allowed"]
...
def _saved_navigation_items(snapshot, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        items.append(
            {
                **(_candidate_view(detail) if detail else {}),
                **row,
                "symbol": symbol,
...
def workspace_bootstrap(background_tasks: BackgroundTasks) -> dict[str, Any]:
    ...
    return {
        ...
        "portfolio_actions": _portfolio_views(snapshot),
...
def latest_intelligence_snapshot(
    ...
    return _full_snapshot_view(snapshot)
    ...
    if wait:
        ...
        return {
            **(service.store.scan(scan["scan_id"]) or scan),
            "snapshot": _full_snapshot_view(snapshot),
        }
    background_tasks.add_task(service.run_scan, scan["scan_id"])
    return scan
...
def instrument_workspace(entity_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "intelligence": _candidate_view(detail),
...
def instrument_evidence(entity_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "host_risk_status": _candidate_view(detail)["host_risk_status"],
        "product_classification": detail.product_classification,
        "product_entry_assessment": product_assessment(detail),
    }
