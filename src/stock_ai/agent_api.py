...
class AgentSettingsUpdate(BaseModel):
    ...
    codex_model: str | None = Field(default=None, max_length=500)
    codex_reasoning_effort: str | None = Field(default=None, max_length=100)
...
def _run_context(request: AgentRunRequest) -> tuple[str, list[str], str]:
    ...
    campaign_requested = objective_completion_contract(objective, None).get("autonomous_pipeline_requested", False)
    explicit_campaign_symbols = []
    if campaign_requested:
        # Revalidate the execution request, not just the optional classifier.
        # A stale/incorrect instrument classification must not turn a market
        # mandate into a trade in the chart's security or a guessed ticker.
        explicit_campaign_symbols = list(dict.fromkeys(
            _literal_taiwan_security_symbols(objective)
            + re.findall(r"(?<![0-9A-Za-z])\d{4,6}\.TW(?:O)?(?![0-9A-Za-z])", objective.upper())
            + _literal_classifier_symbols(objective, request.symbols)
        ))
    ...
    if campaign_requested:
        broad = bool(re.search(r"全市場|整體市場|全台股|whole.market|entire.market|all.market", objective, re.I))
        restrictions = re.findall(r"(?:僅|只|限定|限於|專門|only)\s*(?:為|針對|研究|分析|交易|for|trade)?[^，。；;,\n]{0,60}", objective, re.I)
        restricted = any(_literal_classifier_symbols(clause, explicit_campaign_symbols)
                         or _literal_taiwan_security_symbols(clause) for clause in restrictions) if broad else False
        market_scope = not explicit_campaign_symbols or (broad and not restricted)
    else:
        market_scope = request.context_scope == "market"
    if market_scope:
        scope_note = (
            "[MARKET_SCOPE] This is a whole-market autonomous campaign. Discover and compare securities using broad "
            "market evidence, then choose supported candidates within the authorized paper account. The currently "
            "selected chart security and classifier-invented tickers are not implicit targets.\n\n"
            if campaign_requested else
            "[MARKET_SCOPE] This is a neutral whole-market question. Do not use the currently selected "
        ...
        return (
            model_prefix + scope_note + objective,
            [],
            "market",
        )
    return (
        model_prefix + objective,
        explicit_campaign_symbols if campaign_requested else [str(symbol).strip().upper() for symbol in request.symbols if str(symbol).strip()],
        "instrument" if campaign_requested and explicit_campaign_symbols else request.context_scope or "neutral",
    )
...
def _effective_autonomy(request: AgentRunRequest, objective: str) -> str:
    ...
    the objective forbids. A current, explicit local paper-order or autonomous
    paper-campaign request enters the bounded paper lane; all other stale
    paper selections are reset to advisory. Other explicitly selected
    execution modes remain unchanged.
    """
    contract = objective_completion_contract(objective, None)
    paper_execution_requested = contract["paper_order_requested"] or contract.get("autonomous_pipeline_requested", False)
    if paper_execution_requested and request.autonomy in {"advisory", "paper_execute"}:
...
def _literal_taiwan_security_symbols(objective: str) -> list[str]:
    ...
    for row in rows:
        ...
        for name, matches in (
            ...
            if len(compact_name) < 2 or not _security_name_mentioned(compact_objective, compact_name):
                continue
...
def _security_name_mentioned(question: str, name: str) -> bool:
    """Require a name boundary or an explicit security-targeting context.

    Chinese issuer names are not word-tokenized: 倉和 must not match the
    ordinary phrase 持倉和每日. Ambiguous text stays in the objective for the
    governed search tools rather than assigning a different security.
    """
    for match in re.finditer(re.escape(name), question):
        before, after = question[:match.start()], question[match.end():]
        left_boundary = not before or not re.match(r"[\w\u4e00-\u9fff]", before[-1])
        right_boundary = not after or not re.match(r"[\w\u4e00-\u9fff]", after[0])
        target_prefix = re.search(r"(?:分析|查詢|研究|比較|評估|追蹤|買進|賣出|買入|持有|看好|關注|看看|關於|請問|針對|為|對|買|賣|看|與|和|及)$", before)
        security_suffix = re.match(r"(?:股票|股價|走勢|技術面|基本面|籌碼|財報|股利|公司|營收|的股票|的股價)", after)
        if left_boundary and right_boundary or target_prefix or left_boundary and security_suffix:
            return True
    return False
...
def _literal_classifier_symbols(objective: str, symbols: list[str]) -> list[str]:
    ...
    for raw_symbol in symbols:
        ...
        if re.search(rf"(?<![0-9a-z]){re.escape(symbol.casefold())}(?![0-9a-z])", question) or (
            bare.isdigit()
...
async def _classify_agent_intent(request: AgentIntentClassificationRequest) -> tuple[AgentIntentClassification, str, str | None]:
    ...
    try:
        ...
        read_session_metadata = getattr(provider, "session_metadata", None)
        actual_metadata = read_session_metadata(classifier_id) if callable(read_session_metadata) else {}
    except Exception as exc:
        raise HTTPException(
            status_code=503,
    ...
    execution_contract = objective_completion_contract(request.objective, None)
    if execution_contract["decision_requested"] or execution_contract.get("autonomous_pipeline_requested"):
    ...
    if literal_taiwan_symbols:
        ...
        if (not classification.symbols
                and objective_completion_contract(request.objective, None).get("autonomous_pipeline_requested")):
            classification = classification.model_copy(update={"scope": "market", "use_selected_symbol": False})
    model = actual_metadata.get("model") or provider.capabilities().get("model")
...
@router.get("/providers/codex/models")
async def discover_codex_models() -> dict:
    try:
        return await codex_runtime.model_catalog()
    except CodexAuthenticationRequired as exc:
        raise HTTPException(status_code=401, detail=str(exc)) from exc
    except Exception as exc:
        raise HTTPException(
            status_code=503,
            detail=f"無法讀取 Codex 帳號模型清單：{exc.__class__.__name__}",
        ) from exc
...
async def update_agent_settings(request: AgentSettingsUpdate) -> dict:
    values = request.model_dump(exclude_none=True)
    try:
        if "codex_model" in values or "codex_reasoning_effort" in values:
            current = load_agent_driver_settings()
            model = str(values.get("codex_model", current.codex_model)).strip()
            effort = str(values.get("codex_reasoning_effort", current.codex_reasoning_effort)).strip()
            if values.get("default_driver") == "codex" or (
                model != current.codex_model or effort != current.codex_reasoning_effort
            ):
                await codex_runtime.validate_model_selection(model, effort)
        ...
        raise HTTPException(
            status_code=503, detail=f"無法驗證或儲存 Agent 設定：{exc.__class__.__name__}",
        ) from exc
    clear_agent_service()
