    ...
    *,
    ...
    if include_lifecycle:
        try:
            ...
            logger.warning(
                "Stored security master read failed; falling back to official sources",
                exc_info=True,
            )
...
def securities_master_status(*, refresh: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            if stored_count:
                ...
                return {
                    ...
                    "active": active,
                    "pending_quote": lifecycle.get("by_status", {}).get(
                        "pre_listing",
                        max(0, stored_count - active),
                    ),
                    "lifecycle": lifecycle,
                    ...
                    "data_platform": {
                        ...
                        # The full warehouse diagnostics expand every field's
                        # JSON provenance and take several seconds on a real
                        # data set.  A dashboard read only needs a bounded
                        # availability summary; deep diagnostics remain on the
                        # dedicated platform status endpoint.
                        "warehouse": {
                            "status": "available",
                            "database_path": str(platform.warehouse.path),
            ...
            logger.warning(
                "Stored security master status failed; falling back to official sources",
                exc_info=True,
            )
    if refresh:
        clear_official_caches()
        _cached_margin_rows.cache_clear()
    twse_company_codes = {_clean_text(row.get("公司代號")) for row in twse_companies() if _clean_text(row.get("公司代號"))}
    twse_quote_codes = {_clean_text(row.get("Code")) for row in twse_quotes() if _clean_text(row.get("Code"))}
    tpex_company_codes = {
        _clean_text(row.get("SecuritiesCompanyCode"))
        for row in tpex_companies()
        if _clean_text(row.get("SecuritiesCompanyCode"))
    }
    tpex_quote_codes = {
    ...
    twse_codes = twse_company_codes | twse_quote_codes
    tpex_codes = tpex_company_codes | tpex_quote_codes
    active = len(twse_quote_codes) + len(tpex_quote_codes)
    total = len(twse_codes) + len(tpex_codes)
    platform_status: dict[str, Any]
    lifecycle: dict[str, Any] = {}
    try:
        platform = platform or get_market_data_platform()
        sync = OfficialSecurityMasterLoader(platform).run(force=refresh)
        lifecycle = platform.warehouse.lifecycle_summary()
        platform_status = {
            "status": "succeeded",
            "sync": sync,
            "warehouse": platform.status()["warehouse"],
        }
    except Exception as exc:
        platform_status = {
            "status": "failed",
    ...
    return {
        "count": sum(lifecycle.get("by_entity_type", {}).values()) or total,
        "by_exchange": (
            {
                key: value
                for key, value in (
        ...
        "active": lifecycle.get("by_status", {}).get("active", active),
        "pending_quote": lifecycle.get("by_status", {}).get("pre_listing", total - active),
        "lifecycle": lifecycle,
        "cache": official_cache_status(),
        "refresh_policy": {
            "security_master_seconds": 3600,
...
def securities_for_universe(
    ...
    *,
    ...
    if not symbols:
        return []
    master = {item.symbol.upper(): item for item in list_securities_master(limit=5000)}
    items: list[SecurityMasterItem] = []
