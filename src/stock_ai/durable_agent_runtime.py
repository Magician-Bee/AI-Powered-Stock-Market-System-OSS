...
class DurableAgentRuntime:
    ...
    async def start_background(self) -> None:
        ...
        for run in self.store.recoverable_runs():
            ...
            if run["status"] == "suspended" and error.get("type") != "UserPaused":
                await self.resume(run["run_id"], _automatic_recovery=True)
    ...
    def create_schedule(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        schedule_id = f"AS-{hashlib.sha256(dedup_key.encode()).hexdigest()[:32]}" if dedup_key else f"AS-{uuid4().hex}"
        if dedup_key:
            existing = self.store.get_schedule(schedule_id)
            if existing is not None:
                return existing
            # Keep compatibility with older random schedule IDs. Repeated
            # startup registration must also preserve an explicit pause or a
            # completed one-shot, rather than silently enabling a duplicate.
            for schedule in self.list_schedules(limit=200):
                if str(schedule["payload"].get("dedup_key") or "") == dedup_key:
                    return schedule
        ...
        try:
            return self.store.create_schedule(schedule_id, schedule_payload)
        except sqlite3.IntegrityError:
            # Concurrent registrants share the same durable identity.
            existing = self.store.get_schedule(schedule_id) if dedup_key else None
            if existing is not None:
                return existing
            raise
    ...
    async def _automation_reanalyze(
        ...
        intent_spec = intent.to_semantic_dict() if callable(getattr(intent, "to_semantic_dict", None)) else {
            "goal": goal, "symbol": symbol,
        }
        occurrence_key = f"automation-reanalysis:{event_digest({'intent': intent_spec, 'session_id': session_id, 'source_event_id': source_event_id})}" if source_event_id else None
        child = await self.create_run(
            ...
            run_metadata={
                "automation_trigger_id": source_event_id,
                "source": "automation_reanalysis",
            },
        )
        result = await self.wait(str(child["run_id"]))
        if result.get("status") != "completed":
            raise RuntimeError(f"Automation reanalysis did not complete: {result.get('status') or 'unknown'}")
        decision = result.get("decision")
        structured_decision = decision if isinstance(decision, dict) else {}
        return {
            ...
            "action": structured_decision.get("action"),
            "symbol": structured_decision.get("symbol") or symbol or None,
            "recommendation": (
                structured_decision.get("action") or structured_decision.get("recommendation")
                if isinstance(decision, dict) else decision if isinstance(decision, str) else None
            ),
            "confidence": _automation_confidence(result),
            "confidence_type": structured_decision.get("confidence_type") or "model_self_reported",
            "evidence_ids": [
    ...
    async def resume(self, run_id: str, *, _automatic_recovery: bool = False) -> dict[str, Any] | None:
        snapshot = self.store.get_run(run_id)
        if snapshot is None:
            return None
        self._preserve_root_session_focus(snapshot)
        ...
        if run_id in self._tasks and not self._tasks[run_id].done():
            return snapshot
        resource_boundary = self._has_resource_recovery_boundary(snapshot)
        if _automatic_recovery and resource_boundary:
            return snapshot
        checkpoint = self._restore_checkpoint_for_run(run_id, snapshot)
        if resource_boundary:
            await self._authorize_resource_continuation(run_id)
    ...
    async def continue_after_limit(
        ...
        *,
        ...
        driver_id: str | None = None,
        _automatic_recovery: bool = False,
    ) -> dict[str, Any] | None:
        snapshot = self.store.get_run(run_id)
        if snapshot is None:
            return None
        ...
        resource_boundary = self._has_resource_recovery_boundary(snapshot)
        if _automatic_recovery and resource_boundary:
            raise ValueError("Resource budget exhaustion requires explicit user continuation")
        ...
        await self._append_host_event(
            ...
            "run.continuation_requested",
            {
                ...
                "automatic_recovery": _automatic_recovery,
            },
        )
        task = asyncio.create_task(
            self._execute(run_id, request, resume_state=resume_state),
    ...
    async def add_session_message(
        ...
        *,
        ...
        replacement_objective = (replacement_objective or "").strip() or None
        ...
        replaces_active_root_run = affects_active_root_run and (
            outcome.get("intent") == "hard_steer"
            or (outcome.get("intent") == "correct_fact" and replacement_objective is not None)
        )
        ...
        if affects_active_root_run and outcome.get("intent") == "correct_fact":
            # A fact correction keeps the original objective/authority. The
            # existing executor reads this durable queue before its next turn;
            # a paused run keeps the message pending until explicitly resumed.
            self.store.add_control_message(
                active_run_id,
                control_type="correct_fact",
                payload={"instruction": steering_content, "message_id": message["message_id"],
                         "objective_version_id": outcome.get("objective_version_id")},
            )
    ...
    async def request_replan(self, run_id: str, *, instruction: str) -> dict[str, Any]:
        ...
        if snapshot is None:
            raise KeyError(run_id)
        if snapshot["terminal"] and snapshot["status"] not in {"max_steps_reached", "partially_completed"}:
    ...
    async def _execute(
        ...
        *,
        ...
        try:
            ...
            # This read binds legacy checkpoint controls to their real Run.
            # Never infer ownership from a model-supplied interaction/control ID.
            with self.store._connect() as conn:
                control_ids = [row[0] for row in conn.execute(
                    "select control_id from agent_control_messages where run_id=?", (run_id,),
                )]
            provider_resume_state["provider_context_scope"] = {"control_ids": control_ids}
            from open_stock_ai.agent_runtime.providers.model_metadata import host_review_provider_selection
            selection = host_review_provider_selection(request)
            if selection is not None:
                provider_resume_state["provider_model_selection"] = selection
    ...
    def _has_resource_recovery_boundary(self, snapshot: dict[str, Any]) -> bool:
        """A durable Host spending boundary cannot be repaired by more steps.

        The orchestrator deliberately separates resource boundaries from goal
        gaps. Read its explicit state and the durable event as well: a crash
        or a later recovery projection may lose or replace the result fields.
        Only an explicit Host user-control call acknowledges earlier events.
        """
        result = snapshot.get("result") if isinstance(snapshot.get("result"), dict) else {}
        if result.get("recovery_state") == "resource_boundary_reached":
            return True
        exhausted_gaps = {str(item) for item in result.get("goal_completion_gaps") or []}
        for key in ("completion_validation", "completion_evaluation"):
            validation = result.get(key)
            if isinstance(validation, dict):
                exhausted_gaps.update(str(item) for item in validation.get("remaining_gaps") or [])
        if exhausted_gaps & {"cost_budget_exhausted", "token_budget_exhausted", "execution_guard_blocked"}:
            return True
        exhausted_sequence = authorized_sequence = 0
        for event in self.events(str(snapshot.get("run_id") or "")):
            sequence = int(event.get("sequence") or 0)
            if event.get("type") in {"cost_budget.exhausted", "budget.exhausted", "execution_guard.blocked"}:
                exhausted_sequence = max(exhausted_sequence, sequence)
            elif event.get("type") == "resource_boundary.continuation_authorized":
                authorized_sequence = max(authorized_sequence, sequence)
        return exhausted_sequence > authorized_sequence

    async def _authorize_resource_continuation(self, run_id: str) -> None:
        await self._append_host_event(run_id, "resource_boundary.continuation_authorized", {
            "summary": "使用者明確繼續此任務；保留先前成本與資源上限事件，開始新的有限執行預算。",
            "source": "explicit_user_control", "preserve_prior_spending_receipts": True,
        })
    ...
    def _should_auto_continue_recovery(self, run_id: str) -> bool:
        ...
        if self._has_resource_recovery_boundary(snapshot):
            return False
    ...
    async def _continue_recovery_automatically(self, run_id: str) -> None:
        ...
        try:
            await self.continue_after_limit(run_id, max_steps=next_limit, _automatic_recovery=True)
...
def _automation_confidence(result: dict[str, Any]) -> float:
    ...
    if not math.isfinite(value):
        return 0.0
    # Agent decisions are normalized percentages (including 1 meaning 1%).
    # The older top-level Automation confidence field used a 0..1 fraction.
    if isinstance(decision, dict) or value > 1.0:
        value /= 100.0
