...
class MarketDataWarehouse:
    ...
    def resolve_identifier_candidates(
        ...
        *,
        ...
        if as_of:
            timestamp = normalize_timestamp(as_of, required=True)
            clauses.append("(i.valid_from = '' or i.valid_from <= ?)")
            ...
            clauses.append("i.superseded_at is null")
        with self._connect() as conn:
            rows = conn.execute(
                f"""
                select e.*,
        ...
        *,
        ...
        with self._connect() as conn:
            invalid_warrant_rows = conn.execute(
                """
                select i.rowid, i.valid_from, i.metadata_json
                  from entity_identifiers i
                  join market_entities e on e.entity_id=i.entity_id
    ...
    def list_entities(
        ...
        *,
        ...
        if query:
            clauses.append(
                "(lower(e.canonical_name) like ? or exists (select 1 from entity_identifiers i where i.entity_id=e.entity_id and lower(i.identifier_value) like ?))"
            )
        ...
        with self._connect() as conn:
            rows = conn.execute(
                f"""
                select e.*,
                       (select identifier_value from entity_identifiers i
                         where i.entity_id=e.entity_id and i.identifier_type='display_symbol'
                           and i.superseded_at is null
                         order by i.created_at desc limit 1) as display_symbol
            ...
            for item in revision_rows:
                ...
                transformation = {
                    ...
                    "parameters": {
                        ...
                        "identity_contract": "business_no_then_code_and_name",
                    },
                }
