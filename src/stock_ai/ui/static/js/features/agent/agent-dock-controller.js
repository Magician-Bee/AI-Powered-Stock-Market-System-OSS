(function () {
  ...
  document.addEventListener('stock-ai-active-agent-changed', event => {
    ...
    if (!provider) return;
    runtimeIdentity = { provider, model: detail.model || '', reasoning_effort: detail.reasoning_effort || '' };
  ...
  function identityForRun(run) {
    // The header describes the selected Run, including after completion.
    // Composer preferences describe the next Run and must never relabel an
    // existing receipt when settings are saved or restored.
    if (!run.run_id) return runtimeIdentity;
    const resolved = run.result?.provider_model_metadata || run.provider_model_metadata;
    const provider = resolved?.provider || run.driver || run.driver_id || run.provider || 'Stock AI Runtime';
    return {
      provider,
      model: resolved ? resolved.model || '' : run.model || run.provider_model || '',
      reasoning_effort: resolved ? resolved.reasoning_effort || '' : run.reasoning_effort || '',
    };
  }
  ...
  function renderHeader(state) {
    ...
    if (identity) identity.textContent = [runIdentity.model, runIdentity.reasoning_effort, runIdentity.provider].filter(Boolean).join(' · ');
  ...
  async function submitNow(input = {}, { onAccepted = () => {} } = {}) {
    if (window.AgentRuntimeAPI?.waitSettingsSaved) await window.AgentRuntimeAPI.waitSettingsSaved();
  ...
  async function init() {
    ...
    try {
      ...
      const selectedConfig = provider === 'openai-compatible'
        ...
        : provider === 'external-agent' ? settings.external_agent : settings.codex;
      ...
      runtimeIdentities[provider] = {
        ...
        reasoning_effort: selectedConfig?.reasoning_effort || '',
      };
      runtimeIdentity = runtimeIdentities[provider];
