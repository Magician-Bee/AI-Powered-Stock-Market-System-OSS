(function () {
  ...
  function orderedSteps(state) {
    ...
    const plan = Object.values(state.plans || {}).find(item => (
      item.run_id === active && item.plan_id === state.runs[active]?.plan_id
    ));
    const planNodeIds = new Set((plan?.nodes || []).map(node => String(node.node_id)));
    ...
    rawSteps.forEach(step => {
      ...
      // A mirrored branch step may retain an old lifecycle status after the
      // current PlanGraph supersedes it. Keep the plan node authoritative.
      if (planNodeIds.has(String(prior.node_id || prior.step_id))
        && !planNodeIds.has(String(step.node_id || step.step_id))) return;
  ...
  function render(container, state) {
    ...
    const currentStep = isRunning && steps.find(step => step.status === 'running');
    const nextStep = isRunning && steps.find(step => ['pending', 'ready', 'proposed'].includes(step.status));
    const collapsedStatus = runStatus === 'completed'
      ? '已完成；保留執行與修復紀錄'
      : currentStep
