(function () {
  ...
  function render(container, state) {
    ...
    // Empty symbols are deliberate for an accepted whole-market Run. Neither
    // an old snapshot nor workspace intent may narrow that persisted scope.
    const taskSymbols = marketScopedRun ? [] : (runSymbols.length ? runSymbols : explicitSymbols);
    container.replaceChildren();
    const chips = [
      ['目前頁面', visibleWorkspace, 'workspace'],
      ['目前分頁', visibleTab, 'tab'],
      ['畫面股票', selectedSymbol || '未指定', 'visible-symbol'],
      ['任務股票', marketScopedRun ? '全市場' : (taskSymbols.join('、') || '未指定'), 'task-symbol'],
      ['問題類型', intent.title || (run.request?.metadata?.context_scope === 'market' ? '全市場分析' : '辨識中'), 'task-kind'],
      ['自主權', run.autonomy || run.request?.autonomy || 'advisory', 'autonomy'],
      ['Snapshot', snapshot.snapshot_id || payload.snapshot_id || '新 Run 時建立', 'snapshot'],
    ];
    chips.forEach(([label, value, key]) => {
      const chip = document.createElement('button');
      chip.type = 'button';
      chip.className = 'agent-context-chip';
      chip.dataset.contextKey = key;
