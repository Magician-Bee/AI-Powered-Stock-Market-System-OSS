(function () {
  ...
  function reduce(state, event) {
    ...
    if (actualRunId) {
      ...
      const recoveryFinished = ['run.completed', 'run.failed', 'run.cancelled'].includes(type)
      ...
      next.runs[actualRunId] = {
        ...
        checkpoint_status: ['partially_completed', 'max_steps_reached'].includes(explicitStatus)
          ? explicitStatus
          : previousRun.checkpoint_status,
        recovery_pending: recoveryFinished ? false : pendingRecovery,
        recovery_summary: type === 'recovery.continuation_scheduled'
