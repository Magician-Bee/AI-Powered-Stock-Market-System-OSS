...
const NOTIFICATION_CENTER_UI_TIMEOUT_MS = 4_000;
let notificationCenterRequest = null;

function notificationCenterIsVisible() {
  // setWorkspaceTab writes these route attributes before invoking its loader.
  const route = document.documentElement.dataset;
  return !document.hidden && route.workspace === 'market' && route.workspaceTab === 'monitor';
}

function cancelNotificationCenterRequest(reason) {
  const request = notificationCenterRequest;
  if (!request || request.controller.signal.aborted) return;
  request.cancelReason = reason;
  request.controller.abort();
}

function loadNotificationCenter(symbol = state.symbol) {
  // Bootstrap and chart selection also call this loader. Optional quote/news
  // previews belong to the visible monitor route, not those background paths.
  if (!notificationCenterIsVisible()) {
    cancelNotificationCenterRequest('offscreen');
    return Promise.resolve();
  }
  const normalized = String(symbol || '').trim().toUpperCase();
  if (notificationCenterRequest?.symbol === normalized
      && !notificationCenterRequest.controller.signal.aborted) return notificationCenterRequest.promise;
  cancelNotificationCenterRequest('superseded');
  const request = { symbol: normalized, controller: new AbortController(), cancelReason: null, promise: null };
  notificationCenterRequest = request;
  state.notificationPreviews = [];
  if ($('notificationPreviewBox')) $('notificationPreviewBox').innerHTML = renderEmptyBlock('推播預覽載入中', '正在讀取目前標的的選用通知資料。');
  const timer = window.setTimeout(() => {
    if (notificationCenterRequest === request) cancelNotificationCenterRequest('timeout');
  }, NOTIFICATION_CENTER_UI_TIMEOUT_MS);
  const options = { signal: request.controller.signal };
  request.promise = Promise.allSettled([
    api('/api/notifications/channels', options),
    api(`/api/notifications/previews?symbol=${encodeURIComponent(normalized)}`, options),
  ]).then(([channelsResult, previewsResult]) => {
    if (notificationCenterRequest !== request || !notificationCenterIsVisible()
        || ['offscreen', 'superseded', 'pagehide'].includes(request.cancelReason)) return;
    const failure = result => request.cancelReason === 'timeout'
      ? '通知資料未在 4 秒內回應，請重新開啟監控頁重試。'
      : result.reason?.message || '請稍後重新開啟監控頁重試。';
    if (channelsResult.status === 'fulfilled') renderNotificationChannels(channelsResult.value);
    else if ($('notificationStatusBox')) $('notificationStatusBox').innerHTML = renderEmptyBlock('通知通道載入失敗', failure(channelsResult));
    if (previewsResult.status === 'fulfilled') renderNotificationPreviews(previewsResult.value);
    else if ($('notificationPreviewBox')) $('notificationPreviewBox').innerHTML = renderEmptyBlock('推播預覽載入失敗', failure(previewsResult));
  }).finally(() => {
    window.clearTimeout(timer);
    if (notificationCenterRequest === request) notificationCenterRequest = null;
  });
  return request.promise;
}

const notificationRouteObserver = new MutationObserver(() => {
  if (!notificationCenterIsVisible()) cancelNotificationCenterRequest('offscreen');
});
notificationRouteObserver.observe(document.documentElement, { attributes: true, attributeFilter: ['data-workspace', 'data-workspace-tab'] });
document.addEventListener('visibilitychange', () => {
  if (!notificationCenterIsVisible()) cancelNotificationCenterRequest('offscreen');
  else loadNotificationCenter().catch(() => {});
});
window.addEventListener('pagehide', () => cancelNotificationCenterRequest('pagehide'));
...
if ($('refreshAgentCodexModels')) $('refreshAgentCodexModels').addEventListener('click', discoverAgentCodexModels);
bindAgentCodexPickers();
if ($('agentCodexModel')) $('agentCodexModel').addEventListener('change', changeAgentCodexModel);
if ($('agentCodexReasoningEffort')) $('agentCodexReasoningEffort').addEventListener('change', () => { state.agentSettingsFormDirty = true; renderAgentCodexModels(); });
...
if ($('agentDefaultDriver')) $('agentDefaultDriver').addEventListener('change', () => {
  ...
  if (($('agentDefaultDriver')?.value || '') === 'codex' && !state.agentCodexModelStatus) void discoverAgentCodexModels();
  if (($('agentDefaultDriver')?.value || '') === 'openai-compatible' && $('agentOpenAIBaseUrl')?.value?.trim()) {
    discoverAgentModels().catch(err => { $('agentModelDiscoveryState').textContent = err.message || '模型清單讀取失敗。'; });
  }
});
