...
async function loadAgentRuntimeSettings() {
  ...
  document.dispatchEvent(new CustomEvent('stock-ai-active-agent-changed', {
    detail: window.__stockAIActiveAgent,
  }));
...
async function saveAgentRuntimeSettings() {
  ...
  if ($('agentSettingsStatus')) $('agentSettingsStatus').textContent = 'Agent 設定已儲存。';
  return result;
}
