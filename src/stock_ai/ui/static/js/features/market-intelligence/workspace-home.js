(() => {
  ...
  function renderSummary(payload) {
    ...
    const statValues = {
      marketInvestableCount: summary.investable_count || summary.resolved_count || 0,
  ...
  function detailForSymbol(symbol) {
    return bootstrapPayload?.market_snapshot?.candidate_details?.[String(symbol || '').toUpperCase()] || null;
  }
  ...
  async function selectInstrument(symbol, { source = 'workspace', focusChart = false } = {}) {
    ...
    const context = WorkspaceContextStore.set({
      selection: {
        entity_id: entity?.entity_id || normalized,
        symbol: normalized,
        candidate_id: detail?.symbol || null,
      },
      data: {
        market_snapshot_id: bootstrapPayload?.market_snapshot?.snapshot_id || null,
        latest_quote: detail ? {
  ...
  async function searchOrAskAgent(query) {
    ...
    try {
      ...
      window.AgentDockController?.open();
      node('globalAgentSend')?.click();
      feedback(`「${query}」不是目前資料源可直接開啟的證券；已改由 Agent 解析產業或操作指令。`);
    } catch (error) {
      ...
      const result = await api('/api/alerts', {
        ...
        body: JSON.stringify({
          ...
          rule: {
            ...
            snapshot_id: bootstrapPayload?.market_snapshot?.snapshot_id,
          },
        }),
      });
    ...
    if (actionName === 'paper') {
      const result = await api('/api/paper-trading/preview', {
        ...
        body: JSON.stringify({
          ...
          quantity_lots: 1,
          rationale: `首頁市場快照 ${bootstrapPayload?.market_snapshot?.snapshot_id || ''} 預覽`,
          actor: 'user',
        }),
      });
