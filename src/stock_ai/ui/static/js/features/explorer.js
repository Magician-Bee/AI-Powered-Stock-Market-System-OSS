...
function explorerLifecycleLabel(status) {
  const key = String(status || 'unknown').trim().toLowerCase();
  const labels = {
    active: explorerText('可用行情', 'Active'),
    unknown: explorerText('生命週期待核驗', 'Lifecycle unverified'),
    pre_listing: explorerText('預上市', 'Pre-listing'),
    expired: explorerText('已到期', 'Expired'),
    delisted: explorerText('已下市', 'Delisted'),
    suspended: explorerText('暫停交易', 'Suspended'),
  };
  return labels[key] || explorerText(`狀態：${key}`, `Status: ${key}`);
}
...
async function loadEntities(q='') {
  ...
  if (!query) {
    ...
    if (status) status.textContent = explorerText('等待搜尋', 'Ready to search');
    if (hint) hint.textContent = explorerText(
      '輸入證券代號或名稱查詢正式主檔；系統不會把空白搜尋當成隱含的熱門股清單。',
      'Enter a symbol or name to query the official master; an empty search does not imply a popular-stock list.',
    );
    table.innerHTML = `<div class="row header"><div>${explorerText('代號', 'Symbol')}</div><div>${explorerText('名稱', 'Name')}</div><div>${explorerText('市場', 'Market')}</div><div>${explorerText('操作', 'Action')}</div></div><div class="event"><h4>${explorerText('等待搜尋', 'Ready to search')}</h4><p>${explorerText('請輸入代號或名稱。', 'Enter a symbol or name.')}</p></div>`;
    return { items: [], count: 0, status: 'query_required' };
  }
  ...
  try {
    ...
    if (hint) hint.textContent = query
      ? explorerText(`「${query}」找到 ${count.toLocaleString()} 筆；只有 active 身分可開啟個股圖表。`, `Found ${count.toLocaleString()} matches for “${query}”. Only active identities can open a chart.`)
    ...
    const rows = items.map((entity) => {
      ...
      const lifecycle = String(entity?.lifecycle_status || (entity?.is_active === true ? 'active' : 'unknown')).trim().toLowerCase();
      const chartAvailable = entity?.is_active === true && lifecycle === 'active';
      const lifecycleLabel = explorerLifecycleLabel(lifecycle);
      const rowClass = chartAvailable ? 'row clickable' : 'row';
      const rowSymbol = chartAvailable ? ` data-symbol="${escapeHtml(symbol)}"` : '';
      const action = chartAvailable
        ? `<button data-symbol="${escapeHtml(symbol)}" class="open-detail" type="button">${explorerText('查看K線', 'Open Chart')}</button>`
        : `<button type="button" disabled aria-disabled="true" title="${escapeHtml(lifecycleLabel)}">${explorerText('僅供研究', 'Research only')}</button>`;
      return `<div class="${rowClass}"${rowSymbol}>
        <div>${escapeHtml(symbol)}</div><div>${escapeHtml(name)}</div><div>${escapeHtml(market)}<br/><small>${escapeHtml(lifecycleLabel)}</small></div><div>${action}</div>
      </div>`;
    }).join('');
...
async function refreshSecuritiesMaster() {
  const button = $('refreshSecuritiesMaster');
  const status = $('entityPageStatus');
  const hint = $('entitySearchHint');
  if (!button || button.disabled) return null;
  button.disabled = true;
  button.setAttribute('aria-busy', 'true');
  if (status) status.textContent = explorerText('同步中…', 'Syncing…');
  if (hint) hint.textContent = explorerText(
    '正在從 TWSE、TPEx 官方來源同步證券身分與生命週期；來源失敗會明確保留為部分完成。',
    'Syncing security identities and lifecycle data from official TWSE and TPEx sources; source failures remain explicit partial results.',
  );
  try {
    const result = await api(uiDataApi('/securities/master/refresh'), { method: 'POST' });
    const syncStatus = String(result?.data_platform?.sync?.status || result?.data_platform?.status || 'unknown');
    const count = Number(result?.count || 0);
    if (status) status.textContent = syncStatus === 'succeeded'
      ? explorerText('同步完成', 'Synced')
      : explorerText('部分完成', 'Partially synced');
    if (hint) hint.textContent = syncStatus === 'succeeded'
      ? explorerText(`官方主檔同步完成，共 ${count.toLocaleString()} 個證券實體。`, `Official master sync completed with ${count.toLocaleString()} security entities.`)
      : explorerText(`官方主檔同步狀態：${syncStatus}；已保留可驗證資料，共 ${count.toLocaleString()} 個證券實體。`, `Official master sync status: ${syncStatus}; ${count.toLocaleString()} verified security entities remain available.`);
    const query = String($('entitySearchInput')?.value || '').trim();
    if (query) await loadEntities(query);
    else if ($('entityTable')) $('entityTable').innerHTML = `<div class="event"><h4>${explorerText('同步收據已保存', 'Sync receipt saved')}</h4><p>${explorerText('輸入代號或名稱以查詢剛同步的正式主檔。', 'Enter a symbol or name to query the newly synced official master.')}</p></div>`;
    return result;
  } catch (error) {
    if (status) status.textContent = explorerText('同步失敗', 'Sync failed');
    if (hint) hint.textContent = explorerText(
      `官方主檔同步失敗：${error.message}；沒有以猜測或假資料替代。`,
      `Official master sync failed: ${error.message}; no guessed or synthetic data was substituted.`,
    );
    throw error;
  } finally {
...
function bindEntitySearch() {
  ...
  document.getElementById('refreshSecuritiesMaster')?.addEventListener('click', () => {
    refreshSecuritiesMaster().catch(error => console.warn('Security-master refresh is unavailable', error));
  });
}
