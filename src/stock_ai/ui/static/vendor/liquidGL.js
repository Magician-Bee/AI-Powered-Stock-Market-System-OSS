  ...
  class liquidGLRenderer {
    ...
    _initGL() {
      ...
      const fsSource = `
        ...
        void main(){
          ...
          /*
           * jEPxMgW "Liquid Glass Shader" sampling core. The original 9x9
           * range, half-pixel offsets and lighting constants are preserved;
           * sampleUV is the production bridge to this lens' live page bounds.
           */
          if (u_fooonticLens) {
              ...
              for (float x = -SAMPLE_RANGE; x <= SAMPLE_RANGE; x++) {
                  for (float y = -SAMPLE_RANGE; y <= SAMPLE_RANGE; y++) {
                      vec2 sampleOffset = vec2(x, y) * SAMPLE_OFFSET / u_textureResolution;
                      sum += texture2D(u_tex, sampleUV + sampleOffset);
                      total += 1.0;
                  }
              }
              refrCol = sum / total;
          ...
          vec2 chromaDirection = normalize(p + vec2(0.0001));
          vec2 chromaOffset = chromaDirection * u_chromaticAberration * edge * centreBlend;
          vec4 redSample = texture2D(u_tex, sampleUV + chromaOffset);
          vec4 blueSample = texture2D(u_tex, sampleUV - chromaOffset);
          refrCol.rgb = vec3(redSample.r, refrCol.g, blueSample.b);
          ...
          if (u_fooonticLens) {
            ...
            float gradient =
              clamp((clamp(p.y, 0.0, GRADIENT_RANGE) + GRADIENT_OFFSET) / 2.0, 0.0, 1.0) +
              clamp((clamp(-p.y, GRADIENT_EXTREME, GRADIENT_RANGE) * pow(edge, 2.0) + GRADIENT_OFFSET) / 2.0, 0.0, 1.0);
            final = clamp(refrCol + vec4(pow(edge, 8.0)) * gradient + vec4(pow(edge, 16.0)) * LIGHTING_INTENSITY, 0.0, 1.0);
          }
          ...
          if (u_specular) {
            vec2 pointerLight = vec2(0.5) + vec2(u_tiltY, -u_tiltX) * 0.014;
            vec2 lp1 = pointerLight + vec2(sin(u_time*0.16), cos(u_time*0.21))*0.34;
            vec2 lp2 = pointerLight + vec2(sin(u_time*-0.27+1.5), cos(u_time*0.18-0.5))*0.42;
            float h = 0.0;
            h += smoothstep(0.38,0.0,distance(v_uv, lp1))*0.12;
            h += smoothstep(0.46,0.0,distance(v_uv, lp2))*0.075;
            h += pow(edge, 5.0) * 0.11;
            final.rgb += h;
          }
      ...
      this.u = {
        ...
        fooonticLens: gl.getUniformLocation(this.program, "u_fooonticLens"),
      };
    }
    ...
    async captureSnapshot() {
      ...
      const attemptCapture = async (
        ...
        try {
          const isDocumentSample = this.snapshotTarget.hasAttribute(
            "data-liquid-sample-document"
          );
          const fullW = isDocumentSample
            ? Number(this.snapshotTarget.dataset.sampleWidth) || window.innerWidth
            : this.snapshotTarget.scrollWidth;
          const fullH = isDocumentSample
            ? Number(this.snapshotTarget.dataset.sampleHeight) || window.innerHeight
          ...
          const ignoreElementsFunc = (element) => {
            ...
            const isSampleSource =
              element.hasAttribute("data-liquid-sample-source-root") ||
              element.hasAttribute("data-liquid-sample-source") ||
              !!element.closest(
                "[data-liquid-sample-source-root],[data-liquid-sample-source]"
              );
            const style = window.getComputedStyle(element);
            if (style.position === "fixed" && !isSampleSource) {
              return true;
            }
            return (
          ...
          const snapCanvas = await html2canvas(this.snapshotTarget, {
            ...
            onclone: (clonedDocument) => {
              const sampleRoot = clonedDocument.querySelector(
                "[data-liquid-sample-source-root]"
              );
              if (sampleRoot) {
                sampleRoot.removeAttribute("inert");
                sampleRoot.style.opacity = "1";
                sampleRoot.style.visibility = "visible";
                sampleRoot.style.display = "block";
              }
            },
          });

          this._uploadTexture(snapCanvas);
          document.documentElement.dataset.liquidSnapshotSize =
            `${snapCanvas.width}x${snapCanvas.height}`;
          document.documentElement.dataset.liquidSnapshotSource =
            this.snapshotTarget.matches("[data-liquid-sample-document]")
              ? "live-document-ui"
              : "document";
          try {
            ...
            probeContext.drawImage(snapCanvas, 0, 0, probe.width, probe.height);
            ...
            const variance = count
              ? Math.max(0, sumSquares / count - (sum / count) ** 2)
              : 0;
            document.documentElement.dataset.liquidSnapshotVariance =
              variance.toFixed(2);
            document.documentElement.dataset.liquidSnapshotSamples =
              String(count);
          } catch (probeError) {
            document.documentElement.dataset.liquidSnapshotVariance =
              "unavailable";
    ...
    updateCanvasRegion(sourceCanvas) {
      if (!this.texture || !sourceCanvas || !sourceCanvas.width || !sourceCanvas.height) {
        return false;
      }
      const sourceRect = sourceCanvas.getBoundingClientRect();
      if (!sourceRect.width || !sourceRect.height) return false;
      const snapshotRect = this.snapshotTarget.getBoundingClientRect();
      const liveScrollY = this.snapshotTarget.hasAttribute(
        "data-liquid-sample-document"
      )
        ? window.scrollY
        : 0;
      const x = Math.round((sourceRect.left - snapshotRect.left) * this.scaleFactor);
      const y = Math.round(
        (sourceRect.top + liveScrollY - snapshotRect.top) * this.scaleFactor
      );
      const width = Math.max(1, Math.round(sourceRect.width * this.scaleFactor));
      const height = Math.max(1, Math.round(sourceRect.height * this.scaleFactor));
      if (
        x < 0 ||
        y < 0 ||
      ...
      if (!this._canvasPatchCanvas) {
        this._canvasPatchCanvas = document.createElement("canvas");
      }
      const patchCanvas = this._canvasPatchCanvas;
      patchCanvas.width = width;
      patchCanvas.height = height;
      const patchContext = patchCanvas.getContext("2d");
      patchContext.clearRect(0, 0, width, height);
      patchContext.drawImage(sourceCanvas, 0, 0, width, height);
      if (this.staticSnapshotCanvas) {
        const snapshotContext = this.staticSnapshotCanvas.getContext("2d");
        snapshotContext.drawImage(patchCanvas, x, y);
      }
      const gl = this.gl;
      gl.bindTexture(gl.TEXTURE_2D, this.texture);
      gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, false);
      ...
      document.documentElement.dataset.liquidCanvasPatch =
        `${x},${y},${width},${height}`;
      return true;
    }
    ...
    _renderLens(lens) {
      ...
      const docY = rect.top + liveScrollY - snapshotRect.top;
      ...
      gl.uniform1f(
        this.u.chromaticAberration,
        Math.max(0, Math.min(0.02, lens.options.chromaticAberration || 0))
      );
      ...
      gl.uniform1i(
        this.u.fooonticLens,
        lens.el.classList.contains("nav-active-lens") ? 1 : 0
      );
