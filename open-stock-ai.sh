...
BUNDLE_ID="com.choubee.stockai.liquidglass.instance${PROJECT_INSTANCE_ID}"
...
if command -v shasum >/dev/null 2>&1 &&
  ...
  WORKTREE_FINGERPRINT="$(
    git -C "$PROJECT_ROOT" ls-files -co --exclude-standard -- \
      src macos pyproject.toml uv.lock open-stock-ai.sh build-macos-liquid-glass.sh |
...
LAUNCH_AGENT_LABEL="com.choubee.stockai.${PROJECT_INSTANCE_ID}"
...
if ! "$UV" sync --frozen --no-dev --python 3.12; then
  if [ -x "$VENV_PYTHON" ] && "$VENV_PYTHON" -c 'import stock_ai, uvicorn' >/dev/null 2>&1; then
    printf '\033[33mDependency sync could not reach the network; using the verified local runtime.\033[0m\n'
  else
    fail "Dependency setup failed."
  fi
fi
...
if [ -x "$CODEX_BIN" ]; then
  ...
  export STOCK_AI_CODEX_BIN="$(command -v codex)"
elif curl --proto '=https' --tlsv1.2 -LsSf https://github.com/openai/codex/releases/latest/download/install.sh |
...
if ! launchctl submit -l "$LAUNCH_AGENT_LABEL" -o "$STDOUT_LOG" -e "$STDERR_LOG" -- \
  /bin/sh -c 'cd "$1" || exit 1; shift; exec "$@"' -- "$SERVICE_SOURCE_ROOT" \
  /usr/bin/env \
