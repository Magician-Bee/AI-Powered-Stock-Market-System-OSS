...
let workspace = {{selection: {{symbol: '6538.TWO', explicit_intent_symbols: ['6538.TWO']}}}};
const listeners = {{}};
global.WorkspaceContextStore = {{get: () => workspace}};
global.addEventListener = (name, handler) => {{ listeners[name] = handler; }};
global.document = {{createElement: () => ({{dataset: {{}}, addEventListener() {{}}}})}};
global.AgentDockFormatters = {{redact: value => value}};
const container = {{children: [], replaceChildren() {{ this.children = []; }}, append(chip) {{this.children.push(chip);}}}};
require({json.dumps(str(AGENT / 'agent-context-bar.js'))});
const run = {{run_id: 'AR-market', session_id: 'AS-test', status: 'running', symbols: [],
  request: {{symbols: [], metadata: {{context_scope: 'market'}}}}}};
const state = {{runs: {{'AR-market': run}}, active_run_id: run.run_id, active_session_id: run.session_id}};
const chips = () => Object.fromEntries(container.children.map(chip => [chip.dataset.contextKey, chip.textContent]));
const results = [];
AgentContextBar.render(container, state);
results.push(chips());
// Restoring a completed foreground Run retains its whole-market contract.
state.active_run_id = null;
run.status = 'completed';
AgentContextBar.render(container, state);
results.push(chips());
// A stale environment snapshot cannot override the accepted Run's scope.
state.environment_snapshot = {{payload: {{symbols: ['6538.TWO']}}}};
AgentContextBar.render(container, state);
results.push(chips());
workspace = {{selection: {{symbol: '2330.TW', explicit_intent_symbols: ['2330.TW']}}}};
listeners['stock-ai:workspace-context']();
results.push(chips());
// An instrument Run still displays its own symbol independently of the chart.
run.symbols = ['2887.TW'];
run.request = {{symbols: ['2887.TW'], metadata: {{context_scope: 'instrument'}}}};
state.environment_snapshot = null;
AgentContextBar.render(container, state);
results.push(chips());
...
"""
    ...
    assert [item["task-symbol"] for item in results] == [
        "任務股票：全市場", "任務股票：全市場", "任務股票：全市場",
        "任務股票：全市場", "任務股票：2887.TW",
    ]
    assert [item["visible-symbol"] for item in results] == [
        "畫面股票：6538.TWO", "畫面股票：6538.TWO", "畫面股票：6538.TWO",
        "畫面股票：2330.TW", "畫面股票：2330.TW",
    ]


def test_agent_context_chips_wrap_without_horizontal_clipping():
    ...
    for key in (
        ...
        assert f"'{key}'" in context_bar
    assert "chip.dataset.contextKey = key;" in context_bar
    assert ".agent-context-bar{display:grid;grid-template-columns:repeat(2,minmax(0,1fr))" in css
    assert "overflow-x:hidden" in css
    assert ".agent-context-chip{width:100%;min-width:0" in css
    assert "white-space:normal;overflow-wrap:anywhere" in css
    assert '.agent-context-chip[data-context-key="task-kind"]' in css
    assert '@media(max-width:600px)' in css


def test_full_screen_agent_dock_uses_an_opaque_mobile_surface():
    html = (STATIC / "index.html").read_text(encoding="utf-8")
    css = (STATIC / "css" / "features" / "agent-dock.css").read_text(
        encoding="utf-8"
    )

    assert "agent-dock.css?v=20260909-agent-mobile-surface-v5" in html
    assert "@media(max-width:1023px)" in css
    assert (
        ".agent-dock{inset:0;width:100%;border:0;border-radius:0;"
        "background:var(--panel-solid);backdrop-filter:none;isolation:isolate}"
    ) in css
    assert (
        'html:is([data-ui-theme="pearl"],[data-ui-theme="daylight"]) '
        ".agent-dock{background:linear-gradient(155deg,"
        "rgb(252,254,255),rgb(232,242,250))!important}"
    ) in css
    assert '.agent-dock::before{content:"";position:absolute;z-index:0;inset:0;' in css
    assert "background:rgb(18,18,21);pointer-events:none}" in css
    assert ".agent-dock>*{position:relative;z-index:1}" in css
    assert (
        ":is(html.agent-dock-open body,body:has(#agentDock.is-open))>"
        ".app-shell>:is(.sidebar,.main){display:none!important}"
    ) in css
    assert (
        ":is(.liquid-backdrop,.glass-sample-layer,.liquid-lens-proxy,"
        ".liquid-native-canvas,canvas[data-liquid-ignore]){display:none!important}"
    ) in css
...
def test_agent_header_uses_the_selected_sessions_run_and_provider_identity():
    ...
    assert "run.result?.provider_model_metadata || run.provider_model_metadata" in source
    assert "if (!run.run_id) return runtimeIdentity;" in source
...
def test_portable_launcher_isolates_each_desktop_project_webkit_container():
    ...
    assert 'BUNDLE_ID="io.github.magicianbee.stockai.liquidglass.instance${STOCK_AI_RUNTIME_INSTANCE_ID}"' in desktop_launcher
