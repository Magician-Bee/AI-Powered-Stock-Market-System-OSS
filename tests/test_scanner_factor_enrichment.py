...
def test_scanner_enrichment_uses_persisted_available_warehouse_factors():
    ...
    assert {item["domain"] for item in factors.values()} == {"financials", "flows", "events"}
...
def test_current_official_bulk_inputs_are_visible_but_not_promoted_to_pit():
    ...
    assert (fundamental["domain"], fundamental["dataset"]) == ("revenue", "revenues_monthly")
    assert (chip["domain"], chip["dataset"]) == ("flows", "institutional_flows_daily")
