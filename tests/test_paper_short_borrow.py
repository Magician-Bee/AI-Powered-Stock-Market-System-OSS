...
def test_borrow_fee_cash_change_has_durable_critical_receipt(tmp_path) -> None:
    database = tmp_path / "short-borrow-retention.sqlite"
    store = SQLiteStore(db_path=database)
    retention = ContentAddressedRetentionLedger(store=SQLiteRetentionStore(database))
    oms = PaperOMS(
        store=store,
        account_id="short-borrow-retention",
        initial_cash=10_000.0,
        commission_bps=0.0,
        sell_tax_bps=0.0,
        slippage_bps=0.0,
    ...
    broker.submit(
        _ticket("PB-SHORT-RETAIN", side="short_sell", locate=_locate()),
        _market(),
    )

    accrued = broker.process_market_tick(
        "2330.TW", _market(at="2026-07-17T02:00:00+00:00")
    )["borrow_fees"]

    assert accrued["accrued_fee"] == pytest.approx(0.876712, abs=1e-6)
    restarted = ContentAddressedRetentionLedger(store=SQLiteRetentionStore(database))
    records = [
        item for item in restarted._records.values()
        if item["kind"] == "execution_borrow_fee"
    ]
    assert len(records) == 1
    assert records[0]["critical"] is True
    assert records[0]["payload"] == accrued
