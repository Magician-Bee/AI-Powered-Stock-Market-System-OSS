...
def test_codex_run_reports_server_model_and_restores_selection_before_lifecycle_start():
    actual = {
        "provider": "codex", "model": "server-resolved-model", "reasoning_effort": "ultra",
        "resolution_source": "sdk_thread_start", "selected_model": "",
    }

    class ConfiguredDriver(ScriptedDriver):
        restored = None
        closed = False

        def describe(self):
            return {"model": "configured-placeholder"}

        async def start_run(self, context):
            self.restored = context.state.get("provider_model_metadata")

        def session_metadata(self, run_id):
    ...
    driver = ConfiguredDriver([{
        "state": "complete", "summary": "你好。", "tool_calls": [], "decision": None,
    }])
    runtime = AgentOrchestrator(drivers={"codex": driver}, tools=RecordingTools(), default_driver="codex")
    result = asyncio.run(runtime.run(
        objective="請簡短打招呼", max_steps=1,
        resume_state={"checkpoint": {"payload": {"context_state": {"provider_model_metadata": actual}}}},
    ))
    assert driver.restored == actual
    assert driver.closed is True
    assert result["provider_model_metadata"] == actual
    assert result["model_invocations"][0]["model_id"] == "server-resolved-model"
    assert result["model_invocations"][0]["reasoning_effort"] == "ultra"
...
def test_autonomous_campaign_receipts_are_market_decision_evidence_without_legacy_market_tool():
    trace = [{"ok": True, "tool": "autonomy.research", "result": {
        "schema_version": "open_stock_ai.autonomous_research_cycle.v1",
        "results": [{"symbol": "2330.TW", "candidates": []}],
    }}]

    assert _evidence_requirement_met("market_decision", trace) is True


def test_autonomous_status_and_batch_history_cover_portfolio_and_technical_dimensions():
    objective = "請完整分析全市場的持股、技術、基本面、新聞事件與風險。"
    evidence = {
        "account": {"tool": "autonomy.status", "call_id": "status", "result": {
            "account": {"cash_balance": 1_000_000, "positions": []}}},
        "history": {"tool": "autonomy.evidence", "call_id": "batch", "result": {
            "schema_version": "open_stock_ai.autonomy_evidence_batch.v1",
            "evidence": [{"kind": "price_history", "summary": {"symbol": "2330.TW"}}]}},
    }

    check = _requested_market_evidence_coverage_check(
        objective=objective, task_kind="market_decision", evidence_required=True,
        evidence_catalog=evidence)

    assert check["covered_dimensions"] == ["portfolio", "technical"]
    assert check["missing_dimensions"] == ["fundamental", "external_research", "risk"]
    assert check["required_tools"] == [
        "market.monthly_revenue", "web.research or validated external framework evidence", "market.analyze_symbol"]
...
def test_resumed_run_exposes_resolved_interaction_to_the_provider():
    ...
    session_history = [
        {
            ...
            "run_id": "AR-choice", "session_id": "AS-choice",
        }
    ]
    resume_state = {
        "checkpoint": {
    ...
    result = asyncio.run(
        runtime.run(
            objective="請選擇假設停利方案", run_id="AR-choice", session_id="AS-choice",
            resume_state=resume_state,
