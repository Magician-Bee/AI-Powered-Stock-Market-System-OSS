...
def test_feed_requires_and_verifies_content_addressed_source_receipt(tmp_path) -> None:
    ...
    retained = retention.get(f"settlement-pnl-{result['receipt_sha256']}")
    assert retained is not None
    assert retained["kind"] == "execution_settlement_pnl"
    assert retained["critical"] is True
    assert retained["payload"] == result
...
def test_settlement_feed_requires_same_durable_retention_authority(tmp_path) -> None:
    database = tmp_path / "risk.sqlite"
    control = DurableRiskControlStore(database)
    with pytest.raises(ValueError, match="critical retention ledger"):
        SettlementPnLFeed(control, require_critical_retention=True)
...
def test_trade_store_wires_settlement_feed_into_runtime_path(tmp_path) -> None:
    ...
    assert trades.settlement_pnl_feed.retention_ledger is retention
