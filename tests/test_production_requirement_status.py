...
def test_p0_production_requirement_ledger_is_complete_unique_and_evidence_backed() -> None:
    ...
    assert by_id["A-011"]["status"] == "complete"
    ...
    for requirement_id in ("SEC-003", "SEC-004", "SEC-005", "SEC-006"):
        assert by_id[requirement_id]["status"] == "complete"
        assert by_id[requirement_id]["blockers"] == []
    assert by_id["SEC-009"]["status"] == "complete"
    assert by_id["SEC-009"]["blockers"] == []
    assert "docs/validation/hosted-sec009-offhost-audit-20260908.md" in by_id[
        "SEC-009"
    ]["evidence"]
    assert status["full_release_complete_count"] == 85
...
def test_quant_execution_evidence_gate_is_derived_and_fail_closed() -> None:
    ...
    assert len(status["gates"]["full_release"]["incomplete_requirement_ids"]) == 39
