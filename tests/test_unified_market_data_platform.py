...
def test_entity_registry_resolves_cross_source_aliases_and_historical_code_reuse(
    ...
    assert current_reuse["candidate_count"] == 2
    assert current_reuse["warnings"] == ["historical_identifier_reuse"]
...
def test_official_security_loader_uses_three_resumable_partitions(monkeypatch, tmp_path) -> None:
    platform = _platform(tmp_path)
