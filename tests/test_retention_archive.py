...
def test_hosted_retention_archive_round_trip_is_explicitly_not_years_long(
    ...
    assert (verification / "order-audit-reconstruction-receipt.json").exists()
    assert (verification / "hosted-security-audit-gate-receipt.json").exists()
...
def test_hosted_retention_workflow_uses_fresh_runner_and_truthful_retention() -> None:
    ...
    assert "order-audit-reconstruction-receipt.json" in workflow
    assert "hosted-security-audit-gate-receipt.json" in workflow
