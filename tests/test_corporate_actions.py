...
def test_corporate_action_account_effect_has_durable_critical_receipt(tmp_path, monkeypatch):
    monkeypatch.setenv("OPEN_STOCK_AI_PAPER_INITIAL_CASH", "1000000")
    database = tmp_path / "corporate-action-retention.sqlite"
    store = SQLiteStore(db_path=database)
    retention = ContentAddressedRetentionLedger(store=SQLiteRetentionStore(database))
    oms = PaperOMS(store=store, retention_ledger=retention)
    _buy(oms, "2330.TW")
    ...
    records = [
        ...
        if item["kind"] == "execution_corporate_action"
    ]
    assert len(records) == 1
    assert records[0]["critical"] is True
    assert records[0]["payload"]["schema_version"] == (
        "open_stock_ai.paper_corporate_action_execution.v1"
    )
    assert records[0]["payload"]["application"]["cash_delta"] == 200.0
