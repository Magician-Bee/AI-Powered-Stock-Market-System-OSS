...
def test_candidate_contract_exposes_market_category_factors_and_model_state():
    ...
    assert item.entity_id == "entity:2330.TW"
...
def test_broad_scanner_excludes_etf_from_investable_universe_and_reports_breakdown():
    scanner = BroadScanner(
        feature_loader=lambda: [_feature("2330.TW"), _feature("0050.TW", etf=True)],
        position_loader=lambda: {},
    )
    result = scanner.scan()
    assert [item["symbol"] for item in result["features"]] == ["2330.TW"]
    assert result["universe_breakdown"] == {
...
def test_held_etf_receives_portfolio_action_without_becoming_market_candidate():
    ...
    assert held.portfolio_action == "add"
    assert "0050.TW" in result["portfolio_actions"]["add"]
