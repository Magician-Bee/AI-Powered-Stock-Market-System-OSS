...
def test_frontend_scripts_are_partitioned_and_loaded_in_dependency_order():
    ...
    expected = [
        ...
        "/static/js/features/agent-codex-models.js",
...
def test_market_intelligence_home_workspace_is_bound_without_a_required_symbol_input():
    ...
    for item in [
        ...
        'market-workspace.css?v=20260908-bounded-market-list-v1',
...
def test_system_securities_page_has_a_searchable_native_empty_state():
    ...
    assert 'id="refreshSecuritiesMaster"' in index_html
    assert "function bindEntitySearch()" in explorer_js
    assert "async function refreshSecuritiesMaster()" in explorer_js
    assert "uiDataApi('/securities/master/refresh')" in explorer_js
    assert "{ method: 'POST' }" in explorer_js
    assert "status: 'query_required'" in explorer_js
    assert "空白搜尋" in explorer_js
    ...
    assert "features/explorer.js?v=20260913-catalogue-identity-refresh-v2" in index_html
    ...
    assert "function explorerLifecycleLabel(status)" in explorer_js
    assert "entity?.lifecycle_status" in explorer_js
    assert "entity?.is_active === true && lifecycle === 'active'" in explorer_js
    assert "chartAvailable ? 'row clickable' : 'row'" in explorer_js
    assert "disabled aria-disabled=\"true\"" in explorer_js
    assert "僅供研究" in explorer_js
    assert "只有 active 身分可開啟個股圖表" in explorer_js
