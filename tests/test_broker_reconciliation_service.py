...
def test_reconciliation_receipts_and_event_claims_survive_service_restart(tmp_path) -> None:
    ...
    store = ReconciliationReceiptStore(
        database,
    ...
    restarted_retention = ContentAddressedRetentionLedger(
    ...
    restarted = BrokerReconciliationService(
        ...
        receipt_store=ReconciliationReceiptStore(
            store.path,
            retention_ledger=restarted_retention,
            require_critical_retention=True,
        ),
    ...
    retained = [
        item for item in restarted_retention._records.values()
        if item["kind"] == "execution_reconciliation"
    ]
    assert len(retained) == 3
    assert all(item["critical"] is True for item in retained)
    assert all(restarted_retention.get(item["record_id"]) == item for item in retained)
...
def test_reconciliation_store_requires_same_durable_retention_authority(tmp_path) -> None:
    database = tmp_path / "reconciliation.sqlite"
    with pytest.raises(ValueError, match="critical retention ledger"):
        ReconciliationReceiptStore(database, require_critical_retention=True)
