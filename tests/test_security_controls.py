...
def test_security_control_receipt_covers_sec003_to_sec010_and_is_hash_verified() -> None:
    ...
    assert all(control["external_blockers"] for control in receipt["controls"])
...
def test_security_control_catalog_has_unique_ids_and_external_blockers() -> None:
    ids = [control.identifier for control in SECURITY_CONTROLS]
    assert ids == [f"SEC-{index:03d}" for index in range(3, 11)]
    assert len(set(ids)) == len(ids)
    assert all(control.external_blockers for control in SECURITY_CONTROLS)
